...
# F-68：切换账号时只暂存/合并 Trae 全局项目导航键，不触碰账号凭证和账号专属会话。
$Script:ProjectStateFile = "$Script:AppDataDir\data\trae_relay\current-project-state.json"
# F2-3 双端解耦：WorkBuddy 与 CodeBuddy 切换/保存互不关停对方进程（对端登记/拉回
# 机制 F2-1 已整体移除——实测 CodeBuddy 从不回写共享 auth 文件，无污染风险）。
...
switch ($TargetApp) {
    ...
    'WorkBuddy' {
        ...
        # F2-3 双端完全解耦：auth 文件虽与 CodeBuddy 共用同一物理文件（CodeBuddyExtension
        # 宿主目录），但实测确认 CodeBuddy 从不回写共享 auth 文件——其登录真源在自身
        # %APPDATA%\CodeBuddy CN\User\globalStorage\state.vscdb（vscdb 驱动，F1-2 切换
        # 验证中 auth mtime 恒不变化、客户端启动也不重写 auth）。2026-09-12 观测到的
        # "恢复后 3 秒 auth 被旧账号回写"实为 WorkBuddy 自身强杀后残留子进程的延迟落盘
        # （Wait-AuthFileQuiet 等待静默已兜底），与 CodeBuddy 无关。因此切/存 WorkBuddy
        # 不再关停 CodeBuddy，两端切换/保存完全独立（ProcNames 仅本端）。
...
function Find-ProjectStatePython {
    $candidates = @(
        (Join-Path $PSScriptRoot '..\python\python.exe'),
        (Join-Path $PSScriptRoot '..\src-tauri\target\debug\python\python.exe'),
        (Join-Path $PSScriptRoot '..\src-python\..\src-tauri\target\python-runtime\python.exe')
    )
...
function Find-ProjectStateTool {
    $candidates = @(
        (Join-Path $PSScriptRoot '..\python\trae_project_state.py'),
        (Join-Path $PSScriptRoot '..\src-python\trae_project_state.py'),
        (Join-Path $PSScriptRoot '..\src-tauri\target\debug\python\trae_project_state.py'),
        (Join-Path $PSScriptRoot '..\src-tauri\target\python-runtime\trae_project_state.py')
    )
    foreach ($candidate in $candidates) {
        try {
            $resolved = [System.IO.Path]::GetFullPath($candidate)
...
function Invoke-ProjectStateCapture {
    if ($Script:SnapshotLayout -ne 'icube') { return $false }
    $db = Join-Path $Script:TraeDataDir 'User\globalStorage\state.vscdb'
    if (-not (Test-Path -LiteralPath $db)) {
        Write-Step -Stage 'project' -Message '未找到 state.vscdb，跳过跨账号项目列表保留' -Status 'skip'
        return $false
    }
    $py = Find-ProjectStatePython
    $tool = Find-ProjectStateTool
    if (-not $py -or -not $tool) {
        Write-Step -Stage 'project' -Message '未找到项目状态工具，跳过跨账号项目列表保留' -Status 'warn'
        return $false
    }
    $parent = Split-Path $Script:ProjectStateFile -Parent
    if (-not (Test-Path $parent)) { New-Item -ItemType Directory -Path $parent -Force | Out-Null }
    & $py $tool --capture --db $db --out $Script:ProjectStateFile 2>$null
    if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $Script:ProjectStateFile)) {
        Write-Step -Stage 'project' -Message "切换前项目导航状态提取失败（exit=$LASTEXITCODE），继续切换但不合并项目列表" -Status 'warn'
        return $false
    }
    Write-Step -Stage 'project' -Message '已暂存当前账号的项目列表与最近打开记录（仅全局导航键）' -Status 'ok'
    return $true
}

function Invoke-ProjectStateMerge {
    param([bool]$Captured)
    if (-not $Captured -or $Script:SnapshotLayout -ne 'icube') { return }
    $db = Join-Path $Script:TraeDataDir 'User\globalStorage\state.vscdb'
    if (-not (Test-Path -LiteralPath $db) -or -not (Test-Path -LiteralPath $Script:ProjectStateFile)) { return }
    $py = Find-ProjectStatePython
    $tool = Find-ProjectStateTool
    if (-not $py -or -not $tool) { return }
    & $py $tool --merge --db $db --in $Script:ProjectStateFile 2>$null
    if ($LASTEXITCODE -eq 0) {
        Write-Step -Stage 'project' -Message '已将项目列表与最近打开记录合并回目标账号状态（目标账号已有项优先）' -Status 'ok'
    } else {
        Write-Step -Stage 'project' -Message "目标账号项目导航状态合并失败（exit=$LASTEXITCODE），不影响登录态切换" -Status 'warn'
    }
}
...
# icube 布局启动后登录态确认。
# storage.json 中的 iCubeAuthInfo://icube.cloudide 是 Trae Work 的登录凭据槽位；
# 值本身由客户端加密，桥接层不解密、不记录，只检查条目是否仍存在且非空。
# 这能识别“快照恢复成功但客户端启动后立即清掉失效 JWT”的情况，避免把
# 已被服务端拒绝的账号误报为切换成功。目标 UID 不从密文中推导，账号归属仍由
# 快照目录名和 Rust 侧 vault 维护。
function Confirm-IcubeSwitch {
    param([string]$Slot)
    $storageFile = Join-Path $Script:TraeDataDir 'User\globalStorage\storage.json'
    Write-Step -Stage 'verify' -Message '等待 Trae Work 启动并确认 CloudIDE 登录态（最长 20 秒）…' -Status 'running'
    $deadline = (Get-Date).AddSeconds(20)
    $reported = $false
    while ((Get-Date) -lt $deadline) {
        Start-Sleep -Seconds 2
        if (-not (Test-Path $storageFile)) { continue }
        try {
            $json = Get-Content $storageFile -Raw -Encoding UTF8 | ConvertFrom-Json
            $auth = $json.psobject.Properties | Where-Object {
                $_.Name -eq 'iCubeAuthInfo://icube.cloudide'
            } | Select-Object -First 1
            $value = if ($auth) { [string]$auth.Value } else { '' }
            if ($value.Trim().Length -ge 20) {
                Write-Step -Stage 'verify' -Message "Trae Work 已保留账号 $Slot 的 CloudIDE 登录条目" -Status 'ok'
                return 'ok'
            }
            if (-not $reported) {
                Write-Step -Stage 'verify' -Message 'Trae Work 已启动，但 CloudIDE 登录条目为空，继续等待客户端完成初始化…' -Status 'info'
                $reported = $true
            }
        } catch {
            # 客户端启动期间可能正在原子替换 storage.json，下一轮重试。
        }
    }
    Write-Step -Stage 'verify' -Message "20 秒内未确认账号 $Slot 的 CloudIDE 登录态；可能 JWT 已失效并被客户端清除，请查看 Trae Work 登录状态" -Status 'warn'
    return 'timeout'
}
...
# 批次1：authfile 切换后轮询确认（F-02 验收项，超时 30s）。三信号，命中任一即确认：
...
#   ③vscdb mtime 因果信号（F2-3，仅 CodeBuddy）——CodeBuddy 登录真源在自身 vscdb、
#     实测从不回写 auth 文件，信号②恒不触发导致 verify 恒超时告警。改以客户端启动后
#     实际重写自身登录库（state.vscdb mtime 偏离恢复锚点）计为已接受恢复的登录态。
...
function Confirm-AuthFileSwitch {
    ...
    $vscdbFile = if ($slotHasVscdb -and $Script:CbGlobalStorageDir) { Join-Path $Script:CbGlobalStorageDir 'state.vscdb' } else { $null }
    $vscdbMtime = $null
    if ($vscdbFile -and (Test-Path $vscdbFile)) {
        try { $vscdbMtime = (Get-Item $vscdbFile).LastWriteTimeUtc } catch {}
    }
    if (-not $hasSnapDir) {
        Write-Step -Stage 'verify' -Message "$($Script:AppName) 数据目录无登录快照（storage\skeleton 不存在），改以客户端实际重写信号确认（auth 重写或 vscdb 更新，静默不动不算确认）" -Status 'info'
    } else {
    ...
    while ((Get-Date) -lt $deadline) {
        ...
        $vscdbRewritten = $false
        if ($vscdbMtime) {
            try { $vscdbRewritten = ((Get-Item $vscdbFile).LastWriteTimeUtc -ne $vscdbMtime) } catch {}
        }
        if ($vscdbRewritten -or ($authUid -and $authUid -eq $expectUid -and $authRewritten)) {
            $authHits++
            if ($authHits -ge 2) {
    ...
    if (-not $hasSnapDir) {
        ...
        if ($slotHasVscdb) {
            Write-Step -Stage 'verify' -Message "30 秒内未观察到 $($Script:AppName) 客户端重写登录数据（auth 文件与 vscdb 均无更新——客户端可能未启动或未接受恢复的登录态），请打开客户端核实登录身份" -Status 'warn'
        } else {
...
try {
    ...
    switch ($Action) {
        'Switch' {
            ...
            Stop-Trae
            # F-68：客户端关闭后读取 state.vscdb，避免 WAL/文件锁导致项目导航状态提取不完整。
            $projectStateCaptured = Invoke-ProjectStateCapture
            ...
            # 目标快照恢复完成后，仅合并全局项目导航键；账号凭证和账号分区数据仍以目标快照为准。
            Invoke-ProjectStateMerge -Captured $projectStateCaptured
            # 记录当前账号 ID
            Set-CurrentAccount -AccountId $UserId
            Start-Trae
            # 所有布局都在启动后做确认。icube 只检查加密 storage.json 的 CloudIDE 条目是否
            # 仍存在；authfile 沿用其已有 UID/vscdb 因果校验；chromium 继续使用原有布局流程。
            if ($Script:SnapshotLayout -eq 'icube') {
                $verifyResult = Confirm-IcubeSwitch -Slot $UserId
                if ($verifyResult -eq 'timeout') {
                    Write-Step -Stage 'done' -Message "已恢复账号 $UserId 的快照并启动 Trae Work（警告：登录态未确认，请打开客户端核实）" -Status 'ok'
                } else {
                    Write-Step -Stage 'done' -Message "已切换至账号 $UserId" -Status 'ok'
                }
            } elseif ($Script:SnapshotLayout -eq 'authfile') {
