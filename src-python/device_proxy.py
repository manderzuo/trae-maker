...
# Trae Work CN 的请求不一定使用标准 Authorization，而是把 Cloud-IDE
# 凭证和请求签名放在 x-ide-token/x-helios/x-medusa 等专用头中；这些头
# 同样属于鉴权材料，必须在明文请求日志落盘前掩码。
_SENSITIVE_HEADERS = {
    "authorization", "cookie", "set-cookie", "x-api-key", "proxy-authorization",
    "x-ide-token", "x-cloudide-token", "x-icube-token",
...
def _is_media_body(headers, path=""):
    """媒体/上传请求只记录长度，不把图片、视频或 multipart 内容写入日志。"""
    content_type = ""
    if hasattr(headers, "items"):
        values = headers.items()
    else:
        values = headers or []
    for name, value in values:
        if str(name).lower() == "content-type":
            ...
            break
    if content_type.startswith(("image/", "video/", "audio/", "multipart/")):
        return True
    if "application/octet-stream" in content_type:
        return True
    lower_path = str(path or "").lower()
    return any(term in lower_path for term in (
        "get_resource_upload_token", "get_resource_upload_url",
        "commit_resource_upload_result", "/assets", "/content",
    )) and bool(content_type)


def _safe_body_preview(body, headers, path, limit):
    if not body:
        return ""
    if _is_media_body(headers, path):
        return "[媒体/上传内容已省略，仅记录长度]"
    return _mask_body(body[:limit].decode("utf-8", "replace") if isinstance(body, bytes) else str(body)[:limit])


class ProxyRequestLogger:
    """将代理抓取到的请求/响应摘要记录到明文文件。
    按日命名（proxy_req_%Y-%m-%d.log，同日追加），单日超 100MB 时换 .log.N 序号文件继续追加。
    文件存放在 logs/ 目录下，文件名前缀 proxy_req_ 以区分 proxy.log 操作日志。

    脱敏红线：本文件是明文落盘日志，任何凭证类请求/响应头（包括 Trae Work CN
    的 x-ide-token/x-helios/x-medusa 等专用头）写入前必须经 _mask_header_value 掩码，
    禁止明文落盘；图片、视频和 multipart 体只记录长度，不记录内容。"""
    ...
    def log_request(self, method, host, path, req_headers, req_body, resp_status, resp_reason, resp_headers, resp_body):
        with self._lock:
            ...
            if req_body:
                ...
                lines.append(_safe_body_preview(req_body, req_headers, path, 4096))
            ...
            if resp_body:
                ...
                lines.append(_safe_body_preview(decompressed, resp_headers, path, 8192))
...
# OAuth 浏览器登录使用本机回环回调；系统浏览器不应被本地 MITM 证书拦截。
# 默认只对 Trae 登录域名做透明 CONNECT，不影响 mchost.guru 的原生 API 捕获。
# 设 PROXY_OAUTH_DIRECT=0 可恢复旧行为（仅用于排查兼容性，不建议生产使用）。
OAUTH_DIRECT = os.environ.get("PROXY_OAUTH_DIRECT", "1") not in ("0", "false", "False", "")
OAUTH_DIRECT_DOMAINS = ("trae.cn", "trae.com.cn")
...
KNOWN_TRAE_PATHS = [
    ...
    # Trae Work CN 内置 Seedance 视频插件：原生 GenerateVideo 流式接口。
    # 这里只做诊断分类，不改写请求或响应。
    ("/api/ide/v1/tool_text_to_video_stream", "Seedance视频生成"),
    ("/api/ide/v1/get_resource_upload_token", "视频资源上传令牌"),
    ("/api/ide/v1/get_resource_upload_url", "视频资源上传地址"),
    ("/api/ide/v1/commit_resource_upload_result", "视频资源上传提交"),
    ("/api/ide/v1/get_resource_url", "视频资源地址"),
    ("/api/ide/v1/get_detail_param", "模型列表"),
    ("/api/remote/v1/plugins", "插件接口"),
    ("/api/remote/v1/skills", "技能列表"),
]

# 桌面 Trae Work 自动切号信号：只观察真正的生成/对话端点，绝不对 OAuth、积分查询、
# 上传令牌等辅助请求触发切号。代理只输出一条不含 JWT/响应正文的标记行，实际切换由
# Rust/前端已有的「保存当前登录态 + 切换并继续」流程完成。
AUTO_SWITCH_PATH_HINTS = (
    "/api/agent/v3/llm_utils_chat",
    "/api/ide/v1/tool_text_to_video_stream",
)
AUTO_SWITCH_PATH_IGNORES = (
    "exchangetoken",
    "/oauth/",
    "ide_user_ent_usage",
    "checkin_credits",
    "get_resource_upload_token",
    "get_resource_upload_url",
    "commit_resource_upload_result",
    "get_resource_url",
    "get_detail_param",
)
AUTO_SWITCH_ERROR_TERMS = (
    "积分不足", "余额不足", "额度不足", "超出额度", "超出限额", "资源耗尽",
    "insufficient_quota", "quota_exceeded", "quota exceeded", "quota exhausted",
    "insufficient credit", "credits exhausted", "credit_not_enough", "credit insufficient",
    "resource exhausted", "plan limit", "limit reached", "no quota",
)

# Trae Work 在视频额度预检失败时，后端仍会给 create_agent_task 返回 200，
# 随后由客户端把 error_card_show / code_comp_fail 作为监控事件上报到
# mcs.zijieapi.com/list。该事件才是目前实测到的稳定额度不足信号；只对明确的
# credit_reservation + 4040（视频积分不足）组合触发，避免普通监控事件误切账号。
AUTO_SWITCH_TELEMETRY_HOST = "mcs.zijieapi.com"
AUTO_SWITCH_TELEMETRY_PATH_PREFIX = "/list"


def auto_switch_request_reason(host, path, body=b"", headers=None):
    """检查请求体中的 Trae Work 客户端额度错误事件。

    视频预检失败不是生成接口的 402，而是客户端通过 mcs 监控上报错误卡片。
    返回 ``quota`` 时由 forward_upstream 记录同一条 [AUTO-SWITCH] 标记，
    复用现有 Rust/前端自动切号链路。只扫描固定监控主机和 /list，且不记录正文。
    """
    h = (host or "").lower().rstrip(".")
    p = (path or "").lower()
    if h != AUTO_SWITCH_TELEMETRY_HOST or not p.startswith(AUTO_SWITCH_TELEMETRY_PATH_PREFIX):
        return None
    if not body:
        return None
    raw = body if isinstance(body, bytes) else str(body).encode("utf-8", "replace")
    # 监控 POST 通常为明文 JSON；兼容偶发 gzip/deflate 请求体，不影响正常透传。
    enc = ""
    ...
    has_reservation = "credit_reservation" in text
    has_4040 = '"error_code":"4040"' in text or '"error_code":4040' in text
    has_video_balance = "insufficient balance for video generation" in text
    if has_reservation and (has_4040 or has_video_balance):
        return "quota"
    return None
...
def host_in_oauth_direct(host):
    """OAuth 登录域名走原始 TLS，避免系统浏览器因本地 CA 未安装而报证书错误。"""
    if not OAUTH_DIRECT:
        return False
    h = (host or "").lower().rstrip(".")
    return any(h == d or h.endswith("." + d) for d in OAUTH_DIRECT_DOMAINS)
...
def is_stream_request(path, headers=None):
    """判断请求是否应按 SSE 流式转发。

    Trae Work CN 的 Seedance GenerateVideo 请求有时没有显式携带
    ``Accept: text/event-stream``，但其固定接口仍返回 SSE。集中判断可避免
    代理把视频生成过程全量缓冲后才交给客户端。
    """
    p = path or ""
    h = headers or {}
    accept = ""
    for k, v in h.items():
        if str(k).lower() == "accept":
            ...
            break
    return (
        "/api/agent/v3/llm_utils_chat" in p
        or "/api/ide/v1/tool_text_to_video_stream" in p
        or "event-stream" in accept.lower()
    )


def _response_text_for_signal(body, headers=None):
    """把响应体变成仅用于关键词判断的文本，不写入日志。"""
    if not body:
        return ""
    raw = body if isinstance(body, bytes) else str(body).encode("utf-8", "replace")
    enc = ""
    ...
    try:
        ...
        # 解压失败不影响正常透传；仍尝试扫描原始字节（JSON 通常是明文）。
        pass
    return raw.decode("utf-8", "replace").lower()


def auto_switch_reason(path, status, body=b"", headers=None):
    """返回桌面自动切号原因（auth/quota），否则 None。

    这是纯函数，供代理与单元测试共用。只对 Trae Work 的对话/Seedance 生成接口
    生效，避免积分查询、登录换 token 或上传辅助请求的 401/403 误触发切号。
    """
    p = (path or "").lower()
    if not any(h in p for h in AUTO_SWITCH_PATH_HINTS):
        return None
    if any(h in p for h in AUTO_SWITCH_PATH_IGNORES):
        return None
    try:
        code = int(status)
    except Exception:
    ...
    if code in (401, 403):
        return "auth"
    text = _response_text_for_signal(body, headers)
    if code == 402 or any(term in text for term in AUTO_SWITCH_ERROR_TERMS):
        return "quota"
    return None


def _auth_uid_for_request(headers):
    """从请求 Authorization 提取 uid，仅用于自动切号事件标记。"""
    auth_names = {
        "authorization", "x-ide-token", "x-cloudide-token", "x-icube-token",
        "x-helios", "x-medusa", "x-neptune",
    }
    for k, v in (headers or {}).items():
        if str(k).lower() in auth_names:
            uid = extract_user_id(str(v or ""))
...
def _harden_ca_dir():
    ...
    try:
        ...
        if not username:
            return
        # 目录 ACL 必须带 OI/CI，否则新建的 ca.cer/ca.crt/ca.key 会继承到
        # 一个没有任何 ACE 的 DACL，随后 certutil/UAC 安装进程会读不到证书。
        r = subprocess.run(
            ["icacls", CA_DIR, "/inheritance:r", "/grant:r", f"{username}:(OI)(CI)F"],
            capture_output=True, timeout=15, creationflags=0x08000000)
        if r.returncode == 0:
            # 修复本次生成前已存在的文件：目录 ACL 改好后，旧文件不会自动补回
            # 明确 ACE，因此逐个授予当前用户完全控制；不把私钥暴露给其他账户。
            for name in ("ca.crt", "ca.key", "ca.cer"):
                p = os.path.join(CA_DIR, name)
                if os.path.exists(p):
                    subprocess.run(
                        ["icacls", p, "/inheritance:r", "/grant:r", f"{username}:F"],
                        capture_output=True, timeout=15, creationflags=0x08000000)
...
def _read_stream_chunk(resp, size=8192):
    """读取 SSE 的一小段而不等待凑满 ``size``。

    ``HTTPResponse.read(8192)`` 对 chunked 响应会在返回前不断拼接上游 HTTP
    chunks，直到累计满 8192 字节或收到终止 chunk。Seedance 的进度事件很小、
    间隔约 3 秒，这会把整个视频生成过程卡在代理内部，Trae 客户端随后按超时
    周期重复发起请求。``read1`` 只做一次底层读取，能把每个 SSE 事件及时透传；
    旧版 Python 没有该方法时再退回 ``read``。
    """
...
def _sse_terminal_seen(buffer):
    """判断已转发的 SSE 字节中是否出现终止事件。"""
    if not buffer:
        return False
    try:
...
def _stream_response(resp, resp_headers, client_sock, host, method, path, req_headers, req_body):
    """流式转发 SSE 响应，逐个底层读取并立即发送给客户端。

    Seedance 上游偶尔在已发送 ``event: done`` 后直接关闭 TLS，Python 的
    ``HTTPResponse`` 可能抛出 ``SSLEOFError``。只要终止事件已送达客户端，
    这种 EOF 就视为正常完成，避免 Trae 把本已生成的视频判成空 URI并重试。
    """
    ...
    auto_signal_logged = False
    early_reason = auto_switch_reason(path, resp_status, b"", resp_headers)
    if early_reason:
        log(
            f"  [AUTO-SWITCH] reason={early_reason} status={resp_status} "
            f"uid={_auth_uid_for_request(req_headers)} path={path}"
        )
        auto_signal_logged = True
    ...
    try:
        while True:
            # read1() 不会等到累计满 8192 字节，Seedance 的小 SSE 事件可即时到达
            chunk = _read_stream_chunk(resp, 8192)
            if not chunk:
                break
            ...
            if not terminal_seen:
                terminal_seen = _sse_terminal_seen(logged_body)
        client_sock.sendall(b"0\r\n\r\n")
    except http.client.IncompleteRead as e:
        # 上游在 chunked body 中途断开时，HTTPResponse 会把已读的尾部放到
        # IncompleteRead.partial；先把这段仍有效的 SSE 送给客户端。
        partial = getattr(e, "partial", b"") or b""
        if partial:
            client_sock.sendall(f"{len(partial):x}\r\n".encode("ascii") + partial + b"\r\n")
            total_bytes += len(partial)
            if len(logged_body) < MAX_LOG_BODY:
                logged_body.extend(partial)
            terminal_seen = terminal_seen or _sse_terminal_seen(logged_body)
        interrupted = not terminal_seen
        if interrupted:
            log(f"  [forward] 流式转发中断: IncompleteRead (已转发 {total_bytes} bytes)")
        else:
            log(f"  [forward] 上游在 done 后结束流 (IncompleteRead，视为完成，共 {total_bytes} bytes)")
        try:
            client_sock.sendall(b"0\r\n\r\n")
        except Exception:
            pass
    except Exception as e:
        # 某些上游 TLS 实现会在最后一个 SSE 事件后以未完成 TLS close
        # 结束连接。终止事件已经交给 Trae 时，这个 EOF 不应触发重试。
        interrupted = not terminal_seen
        if interrupted:
            log(f"  [forward] 流式转发中断: {type(e).__name__}: {e} (已转发 {total_bytes} bytes)")
        else:
            log(f"  [forward] 上游在 done 后结束流 ({type(e).__name__}，视为完成，共 {total_bytes} bytes)")
        try:
            client_sock.sendall(b"0\r\n\r\n")
        except Exception:
            pass
    ...
    if not auto_signal_logged:
        stream_reason = auto_switch_reason(path, resp_status, bytes(logged_body), resp_headers)
        if stream_reason:
            log(
                f"  [AUTO-SWITCH] reason={stream_reason} status={resp_status} "
                f"uid={_auth_uid_for_request(req_headers)} path={path}"
            )
...
def forward_upstream(host, port, method, path, headers, body, client_sock):
    ...
    ctx = ssl.create_default_context()
    # 检测是否为流式请求（SSE）：对话和 Seedance 视频生成均使用长超时。
    is_stream = is_stream_request(path, headers)
    ...
    try:
        ...
        # Trae Work 视频额度预检失败时，create_agent_task 仍返回 200；客户端随后
        # 会把 error_card_show(code=4040, credit_reservation) 上报到 mcs。请求体
        # 诊断必须在响应透传前发出，以便复用已有 [AUTO-SWITCH] → Rust/前端链路。
        request_reason = auto_switch_request_reason(host, path, body, headers)
        if request_reason:
            log(
                f"  [AUTO-SWITCH] reason={request_reason} status={resp.status} "
                # 客户端监控的 /list 本身不代表资源类型；该组合只由视频额度预检
                # 产生，因此给 Rust/前端使用 Seedance 的 Work 积分优先级。
                f"uid={_auth_uid_for_request(headers)} path=/api/ide/v1/tool_text_to_video_stream "
                f"source=client-telemetry original_path={path}"
            )
        ...
        resp_body = resp.read()
        auto_reason = auto_switch_reason(path, resp.status, resp_body, resp_header_pairs)
        if auto_reason:
            log(
                f"  [AUTO-SWITCH] reason={auto_reason} status={resp.status} "
                f"uid={_auth_uid_for_request(headers)} path={path}"
            )
...
def handle_client(conn, addr):
    try:
        ...
        if method == "CONNECT":
            ...
            if host_in_oauth_direct(host):
                # OAuth 浏览器流量透明转发：保留真实证书，授权结果经 127.0.0.1:17388 回环监听接收。
