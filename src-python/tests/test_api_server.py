...
class ApiTester:
    ...
    def test_health(self):
        ...
        try:
            ...
            self.check("不包含 pool 字段", "pool" not in data, f"keys={list(data.keys())}")
            self.check("不包含积分字段", "total_credits" not in data, f"keys={list(data.keys())}")
