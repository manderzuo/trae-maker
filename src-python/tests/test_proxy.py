...
def test_auto_switch_reason_only_generation_endpoints():
    chat = "/api/agent/v3/llm_utils_chat"
    video = "/api/ide/v1/tool_text_to_video_stream"
    assert dp.auto_switch_reason(chat, 401) == "auth"
    assert dp.auto_switch_reason(video, 402) == "quota"
    assert dp.auto_switch_reason(chat, 200, b'{"message":"insufficient_quota"}') == "quota"
    # 辅助接口的错误不能触发桌面切号
    assert dp.auto_switch_reason("/api/ide/v1/get_resource_upload_token", 401) is None
    assert dp.auto_switch_reason("/oauth/ExchangeToken", 401) is None
    assert dp.auto_switch_reason("/trae/api/v2/ug/checkin_credits/status", 401) is None


def test_auto_switch_request_reason_video_credit_reservation_telemetry():
    # Trae Work 实测：视频额度预检失败后由 mcs 监控接口上报错误卡片，
    # 而非由 create_agent_task 返回 402。该请求体是 JSON 数组，错误字段
    # 位于 params/error_message 的转义字符串中，因此按字面组合识别即可。
    body = (
        b'[{"events":[{"event":"error_card_show","params":"'
        b'{\\"card_type\\":\\"credit_reservation\\",'
        b'\\"error_code\\":4040,'
        b'\\"error_message\\":\\"Insufficient balance for video generation.\\"}"}]}]'
    )
    assert dp.auto_switch_request_reason("mcs.zijieapi.com", "/list", body) == "quota"
    assert dp.auto_switch_request_reason("mcs.zijieapi.com", "/list?aid=932204", body) == "quota"
    # 普通 mcs 监控事件、其他主机或非监控路径不能误触发切号。
    assert dp.auto_switch_request_reason("mcs.zijieapi.com", "/list", b'{"error_code":4040}') is None
    assert dp.auto_switch_request_reason("api5-normal.mchost.guru", "/list", body) is None
    assert dp.auto_switch_request_reason("mcs.zijieapi.com", "/other", body) is None


def test_oauth_domains_bypass_mitm_by_default():
    assert dp.host_in_oauth_direct("www.trae.cn")
    assert dp.host_in_oauth_direct("api.trae.cn")
    assert dp.host_in_oauth_direct("login.trae.com.cn")
    assert not dp.host_in_oauth_direct("trae-api-cn.mchost.guru")


def test_media_payload_is_omitted_from_proxy_preview():
    assert dp._is_media_body({"Content-Type": "image/png"}, "/upload")
    assert dp._is_media_body([("content-type", "application/octet-stream")], "/upload")
    assert dp._safe_body_preview(b"secret-image-bytes", {"content-type": "image/png"}, "/upload", 4096) == "[媒体/上传内容已省略，仅记录长度]"
    assert "secret-image-bytes" not in dp._safe_body_preview(b"secret-image-bytes", {"content-type": "image/png"}, "/upload", 4096)
...
def forward_stream_logged(host, port, method, path, headers, body, client_sock, seq, t0):
    ...
    try:
        ...
        while True:
            # 与正式代理保持一致：read1() 只做一次底层读取，避免小 SSE 事件
            # 累积到 8192 字节后才返回，导致观测脚本看起来像请求卡住。
            chunk = dp._read_stream_chunk(resp, 8192)
            if not chunk:
                break
...
def tunnel_https_logged(tls, host, port):
    while True:
        ...
        is_stream = dp.is_stream_request(path, headers)
        if is_stream:
