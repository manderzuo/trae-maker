...
use crate::{admin_session::{ensure_initial_admin_credential, AdminSessionStore, LoginThrottle, SESSION_TTL_MS}, bridge_client::BridgeClient, config::RouterConfig};
...
impl StarlinkRouterState {
    pub fn open(config: RouterConfig, bridge: BridgeClient) -> Result<Arc<Self>, String> {
        ...
        Ok(Arc::new(Self { store, bridge: Arc::new(Mutex::new(bridge)), config, jobs: Arc::new(Mutex::new(jobs)), admin_sessions: Arc::new(AdminSessionStore::new(SESSION_TTL_MS)), login_throttle: Arc::new(LoginThrottle::new()) }))
    }

    pub fn for_test(store: Arc<CoreStore>, bridge: BridgeClient, config: RouterConfig) -> Arc<Self> {
        Arc::new(Self { store, bridge: Arc::new(Mutex::new(bridge)), config, jobs: Arc::new(Mutex::new(HashMap::new())), admin_sessions: Arc::new(AdminSessionStore::new(SESSION_TTL_MS)), login_throttle: Arc::new(LoginThrottle::new()) })
    }
