...
export const api = {
  env: {
    ...
    /** F-67：列出本机隔离的 Trae Work 实例（仅元数据） */
    traeInstancesList: () => invoke<TraeInstanceView[]>('trae_instances_list'),
    /** F-67：创建独立 user-data-dir；seedSnapshot 仅在用户显式选择时复制本机快照 */
    traeInstanceCreate: (instanceId: string, name: string | null, accountUid: string, seedSnapshot: boolean) =>
      invoke<TraeInstanceView>('trae_instance_create', {
        instanceId,
        name,
        accountUid,
        seedSnapshot,
      }),
    traeInstanceStart: (instanceId: string, proxyPort?: number | null) =>
      invoke<TraeInstanceView>('trae_instance_start', { instanceId, proxyPort: proxyPort ?? null }),
    traeInstanceStop: (instanceId: string) =>
      invoke<TraeInstanceView>('trae_instance_stop', { instanceId }),
    traeInstanceForget: (instanceId: string) => invoke<void>('trae_instance_forget', { instanceId }),
  },
  cert: {
  ...
  proxy: {
    ...
    simulateAutoSwitch: () => invoke<void>('proxy_auto_switch_simulate'),
  },
  accounts: {
    ...
    // BitBrowser：扫描配置为 Trae Work 的 profile，并通过 CDP 读取页面 localStorage token
    discoverBitBrowser: () => invoke<DiscoveredAccount[]>('bitbrowser_accounts_discover'),
    profilesBitBrowser: () => invoke<BitBrowserProfile[]>('bitbrowser_profiles_list'),
    importBitBrowser: (windowId: string, expectedUserId?: string, accountName?: string) =>
      invoke<BitBrowserImportResult>('bitbrowser_account_import', {
        windowId,
        expectedUserId: expectedUserId ?? null,
        accountName: accountName ?? null,
      }),
    renewBitBrowser: (userId: string) =>
      invoke<BitBrowserImportResult>('bitbrowser_account_renew', { userId }),
  ...
  oauth: {
    ...
    pollCallback: () => invoke<string | null>('oauth_poll_callback'),
    ...
    loginBitBrowser: (windowId: string, accountName?: string, groupId?: string) =>
      invoke<OAuthLoginResult>('oauth_login_bitbrowser', {
        windowId,
        accountName: accountName ?? null,
  ...
  apiServer: {
    ...
    /** 切换登录态前等待网关生成请求归零；服务未运行时立即返回 */
    waitIdle: (timeoutMs?: number) => invoke<void>('api_server_wait_idle', { timeoutMs: timeoutMs ?? null }),
    ...
    // Seedance 是 Claude Code 的异步 MCP 工具，不作为文字 provider 注册
    ccSwitchRegisterSeedance: (apiKey?: string) =>
      invoke<string>('ccswitch_register_seedance_mcp', {
        apiKey: apiKey ?? null,
        port: null,
      }),
    ...
    conversationsList: () =>
      invoke<ConversationSummary[]>('api_conversations_list'),
    conversationsExport: () =>
      invoke<ConversationExportResult>('api_conversations_export'),
    conversationsSettingsGet: () =>
      invoke<ConversationSettings>('api_conversations_settings_get'),
    conversationsSettingsSet: (settings: ConversationSettings) =>
      invoke<ConversationSettings>('api_conversations_settings_set', { settings }),
    conversationsDelete: (conversationId: string) =>
      invoke<boolean>('api_conversations_delete', { conversationId }),
    ...
    tunnelSave: (config: TunnelConfig) => invoke<TunnelConfig>('gateway_tunnel_save', { config }),
    tunnelStatus: () => invoke<TunnelStatus>('gateway_tunnel_status'),
    tunnelStart: (config: TunnelConfig) => invoke<TunnelStatus>('gateway_tunnel_start', { config }),
    tunnelStop: () => invoke<void>('gateway_tunnel_stop'),
    frpGet: () => invoke<FrpConfig>('gateway_frp_get'),
    frpSave: (config: FrpConfig) => invoke<FrpConfig>('gateway_frp_save', { config }),
    frpStatus: () => invoke<FrpStatus>('gateway_frp_status'),
    frpStart: (config: FrpConfig) => invoke<FrpStatus>('gateway_frp_start', { config }),
    frpStop: () => invoke<void>('gateway_frp_stop'),
  ...
  traeRelay: {
    ...
    capture: (targetApp?: 'TraeWork' | 'Trae') =>
      invoke<TraeRelayPackage>('trae_relay_capture', { targetApp: targetApp ?? null }),
    /** 按 Trae 原生 local_conversation 协议创建可在切号后继续的分享链接 */
    createShare: (sessionId?: string | null, targetApp?: 'TraeWork' | 'Trae') =>
      invoke<TraeShareResult>('trae_relay_create_share', {
        sessionId: sessionId ?? null,
        targetApp: targetApp ?? null,
      }),
    /** 只允许打开 share.traecontent.cn/share/ 下的链接 */
    openShare: (shareUrl: string) =>
      invoke<void>('trae_relay_open_share', { shareUrl }),
    /** 切号后以原生客户端重新打开项目目录 */
    openProject: (projectPath: string, targetApp?: 'TraeWork' | 'Trae', proxyPort?: number) =>
      invoke<void>('open_trae_project', {
        projectPath,
        targetApp: targetApp ?? null,
...
export interface AutoSwitchRequestEvent {
  /** 由代理从 Trae Work Authorization 推导的当前账号，无法推导时为 null */
  currentUid: string | null;
  /** auth=鉴权失效，quota=额度不足 */
  reason: 'auth' | 'quota' | string;
  status: number;
  path: string;
  /** 本地演练事件：只预览候选账号，不执行切换 */
  simulated?: boolean;
}
...
export interface ListenerHandlers {
  ...
  onAutoSwitchRequest?: (e: AutoSwitchRequestEvent) => void;
...
export async function setupListeners(
  ...
  if (handlers.onAutoSwitchRequest) {
    unsubs.push(
      await listen<AutoSwitchRequestEvent>('auto-switch-request', (e) =>
        handlers.onAutoSwitchRequest!(e.payload),
      ),
    );
  }
