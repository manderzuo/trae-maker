...
export interface AccountView {
  ...
  /** 登录态来源：首次 BitBrowser 导入后优先走原生 refresh_token */
  credential_source?: string | null;
}

/** Trae Work CN 切号接力包（不包含 JWT/Cookie 等凭证） */
export interface TraeRelayPackage {
  schema_version: number;
  captured_at: string;
  source_uid: string | null;
  session_id?: string | null;
  target_app: 'TraeWork' | 'Trae' | string;
...
export interface TraeShareResult {
  session_id: string;
...
export interface DiscoveredAccount {
  ...
  in_pool: boolean;
  storage_path: string;
  /** 来源：local = 本机应用数据，bitbrowser = BitBrowser Trae Work 页面 */
  source?: 'local' | 'bitbrowser' | string;
  /** BitBrowser profile 标识（仅 source=bitbrowser） */
  window_id?: string | null;
  window_seq?: number | null;
  window_name?: string | null;
  page_url?: string | null;
  /** 页面 localStorage 是否捕获到 Cloud-IDE-Token */
  token_present?: boolean;
  token_exp_timestamp?: number | null;
  read_error?: string | null;
}

/** Trae Work CN 独立实例（F-67）；不包含 JWT/Cookie 等凭证。 */
export interface TraeInstanceView {
  id: string;
  name: string;
  account_uid: string;
  data_dir: string;
...
/** BitBrowser profile 导入结果；完整 JWT 只在 Rust vault 内流转，不回传前端。 */
export interface BitBrowserImportResult {
  user_id: string;
  name: string;
  updated: boolean;
  token_exp_timestamp: number | null;
  refresh_token_captured: boolean;
  /** 首次导入是否已自动生成原生 TRAE Work CN 切换快照 */
  native_snapshot_created?: boolean;
  /** 已有快照是否已用本次 JWT 更新 */
  native_snapshot_updated?: boolean;
  /** 快照生成失败原因（账号仍已导入，可通过 BitBrowser 续期） */
  native_snapshot_error?: string | null;
}

/** BitBrowser profile 的安全视图（不含密码/Cookie）。 */
...
export interface Settings {
  ...
  /** Trae Work 桌面端额度/鉴权失败时自动切换账号（默认关闭） */
  desktop_auto_switch: boolean;
  /** 自动切号去抖冷却时间（秒） */
  desktop_auto_switch_cooldown_secs: number;
}
...
/** FRP 应用层隧道；frpc 运行在 AI Work 主机，frps/Nginx 在局域网中转机或云端。 */
...
export interface CcSwitchStatus {
  ...
  /** Seedance MCP 是否已注册并启用给 Claude Code */
  seedanceMcpRegistered: boolean;
}
...
export interface OAuthLoginResult {
  ...
  has_refresh_token: boolean;
  token_exp_timestamp?: number | null;
  refresh_exp_timestamp?: number | null;
  credential_source?: string | null;
}
