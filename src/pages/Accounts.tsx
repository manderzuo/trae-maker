...
export default function Accounts() {
  ...
  const switchAndContinue = useAppStore((s) => s.switchAndContinue);
  const relayActive = useAppStore((s) => s.relayActive);
  ...
  const oauthLoginBitBrowser = useAppStore((s) => s.oauthLoginBitBrowser);
  ...
  // F-08/F-75 自动发现：扫描本机应用 + BitBrowser 中 Trae Work 的登录账号
  ...
  const busy = (!!switchingTo || !!savingLogin || relayActive) && !lockTimedOut;
  useEffect(() => {
  ...
  const runDiscover = async () => {
    ...
    setScanning(true);
    try {
      // BitBrowser 未运行时不应影响本机 Trae 扫描，因此分别处理两个结果。
      const [localResult, bitResult] = await Promise.allSettled([
        api.accounts.discover(),
        api.accounts.discoverBitBrowser(),
      ]);
      const local = localResult.status === 'fulfilled' ? localResult.value : [];
      const bit = bitResult.status === 'fulfilled' ? bitResult.value : [];
      setDiscovered([...local, ...bit]);
      if (bitResult.status === 'rejected') {
        toast('info', `BitBrowser 未读取：${String(bitResult.reason)}（本机账号结果仍可用）`);
      }
    } catch (err) {
      setDiscovered([]);
      toast('error', `扫描账号失败：${String(err)}`);
    } finally {
      setScanning(false);
    }
  };
  ...
  const addDiscovered = async (d: DiscoveredAccount) => {
    if (addingUid) return;
    if (d.source === 'bitbrowser' && !d.window_id) {
      toast('error', 'BitBrowser 结果缺少窗口标识，无法导入');
      return;
    }
      ...
      if (d.source === 'bitbrowser') {
        ...
        const snapshotNote = result.native_snapshot_error
          ...
          : result.native_snapshot_created
            ? '；原生切换快照已自动生成'
            : result.native_snapshot_updated
              ? '；原生切换快照已更新'
              : '';
        toast(
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        ...
        actions={
          <>
            ...
            <button onClick={() => void runDiscover()} className="btn-outline" title="扫描本机 Trae 应用与 BitBrowser Trae Work profile">
              <ScanSearch size={15} /> 扫描本机 + BitBrowser
            </button>
                  ...
                  <tr key={a.user_id} className="row-hover border-t border-slate-200 dark:border-zinc-800">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex justify-end gap-1">
                        ...
                        {(a.jwt_exp_hours === null || a.jwt_exp_hours <= 24 || a.cooldown_type === 'SessionDead') && (
                          <button
                            title={
                              switchingTo || savingLogin
                                ? '切换/保存进行中，暂不能续期'
                                : a.credential_source === 'bitbrowser'
                                  ? '续期 JWT（原生续期；旧账号需重新从 BitBrowser 导入一次以接管 refresh_token）'
                                  : a.credential_source == null
                                    ? '续期 JWT（使用原生 Trae Work CN refresh_token）'
                                    : '续期 JWT（使用原生 Trae Work CN refresh_token）'
                            }
                            onClick={() => void renewJwt(a.user_id, a.credential_source)}
                            disabled={busy}
                        ...
                        <button
                          title={relayActive ? '正在切换并接力工作…' : '切换并继续工作（保存当前项目后切换账号）'}
                          onClick={() => void switchAndContinue(a.user_id, 'TraeWork')}
                          disabled={busy || relayActive}
                          className={`btn-ghost !p-2 text-violet-500 hover:bg-violet-50 disabled:cursor-not-allowed disabled:opacity-40 dark:hover:bg-violet-500/10 ${relayActive ? 'animate-pulse' : ''}`}
                        >
                          {relayActive ? <Loader2 size={14} className="animate-spin" /> : <Repeat2 size={14} />}
                        </button>
                        <button
      ...
      <OAuthLoginModal
        ...
        onBitBrowserLogin={async (windowId, accountName, groupId) => {
          try {
            await oauthLoginBitBrowser(windowId, accountName, groupId);
            setOAuthOpen(false);
          } catch {
            /* toast 已发出 */
          }
