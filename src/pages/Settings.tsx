...
import { Calendar, Trash2, Save, Search, RotateCcw, Fingerprint, Clock, AlertTriangle, PlayCircle, Plus, Play, Square, RefreshCw } from 'lucide-react';
...
export default function Settings() {
  ...
  const simulateAutoSwitch = useAppStore((s) => s.simulateAutoSwitch);
  const relayProgress = useAppStore((s) => s.relayProgress);
  ...
  const [simulatingAutoSwitch, setSimulatingAutoSwitch] = useState(false);
  const [instances, setInstances] = useState<import('../types').TraeInstanceView[]>([]);
  const [instanceId, setInstanceId] = useState('');
  const [instanceName, setInstanceName] = useState('');
  const [instanceAccount, setInstanceAccount] = useState('');
  const [seedSnapshot, setSeedSnapshot] = useState(false);
  ...
  const runAutoSwitchSimulation = async () => {
    setSimulatingAutoSwitch(true);
    try {
      await simulateAutoSwitch();
    } finally {
      setSimulatingAutoSwitch(false);
    }
  };
  ...
  useEffect(() => {
    if (!instanceAccount && accounts.length > 0) setInstanceAccount(accounts[0].user_id);
  }, [accounts, instanceAccount]);

  const loadInstances = async () => {
    setLoadingInstances(true);
    try {
      setInstances(await api.env.traeInstancesList());
    } catch (e) {
      toast('error', `加载独立实例失败：${String(e)}`);
    } finally {
  ...
  const createInstance = async () => {
    ...
    if (!/^[A-Za-z0-9_-]{1,64}$/.test(id)) {
      toast('error', '实例 ID 只能包含字母、数字、连字符和下划线');
      return;
    }
    if (!instanceAccount) {
      toast('error', '请先选择账号');
    ...
    try {
      await api.env.traeInstanceCreate(id, instanceName.trim() || null, instanceAccount, seedSnapshot);
      toast('success', `独立实例 ${id} 已创建`);
      setInstanceId('');
      setInstanceName('');
      setSeedSnapshot(false);
      await loadInstances();
    } catch (e) {
      toast('error', `创建独立实例失败：${String(e)}`);
    } finally {
  ...
  const startInstance = async (id: string) => {
    setInstanceBusy(id);
    try {
      const next = await api.env.traeInstanceStart(id, proxy.running ? proxy.port : null);
      setInstances((prev) => prev.map((i) => (i.id === id ? next : i)));
      toast('success', `实例 ${id} 已启动`);
    } catch (e) {
      toast('error', `启动实例失败：${String(e)}`);
    } finally {
  ...
  const stopInstance = async (id: string) => {
    setInstanceBusy(id);
    try {
      const next = await api.env.traeInstanceStop(id);
      setInstances((prev) => prev.map((i) => (i.id === id ? next : i)));
      toast('success', `实例 ${id} 已停止`);
    } catch (e) {
      toast('error', `停止实例失败：${String(e)}`);
    } finally {
      setInstanceBusy(null);
    }
  };

  const forgetInstance = async (id: string) => {
    setInstanceBusy(id);
    try {
      await api.env.traeInstanceForget(id);
      toast('info', `已移除实例 ${id} 的登记（数据目录保留）`);
      await loadInstances();
    } catch (e) {
      toast('error', `移除实例失败：${String(e)}`);
    } finally {
      setInstanceBusy(null);
    }
  };
      ...
      <div className="grid items-start gap-4 md:grid-cols-2">
        ...
        <section className="card p-4">
          ...
          <div className="space-y-3 text-sm">
            ...
            <div>
              <label className="label">BitBrowser Local API 地址</label>
              <input
                type="text"
                value={form.bitbrowser_api_url}
                onChange={(e) => update('bitbrowser_api_url', e.target.value.trim())}
                placeholder="http://127.0.0.1:54345"
                className="input w-full"
              />
              <p className="mt-1 text-xs text-slate-400">
                用于扫描 BitBrowser 中的 Trae Work profile；仅允许本机回环地址，BitBrowser 需开启 Local API。
              </p>
            </div>
          </div>
          ...
          <h3 className="mb-1 font-medium">Trae Work 独立实例</h3>
          <p className="mb-3 text-xs text-slate-400">
            每个实例使用独立的 user-data-dir，可并行打开多个账号。默认创建空目录；只有勾选后才从本机已有快照复制登录态，实例数据不会上传。
          </p>
          <div className="grid grid-cols-1 gap-2 text-sm sm:grid-cols-3">
            <input
              value={instanceId}
              onChange={(e) => setInstanceId(e.target.value)}
              placeholder="实例 ID，如 account-a"
              className="input"
              maxLength={64}
            />
            <input
            ...
            >
              {accounts.length === 0 ? (
                ...
                accounts.map((a) => (
                  <option key={a.user_id} value={a.user_id}>{a.name} · {a.user_id}</option>
                ))
              )}
            </select>
          </div>
          <div className="mt-2 flex flex-wrap items-center gap-3 text-xs text-slate-500 dark:text-zinc-400">
            <label className="flex items-center gap-2">
              <input type="checkbox" checked={seedSnapshot} onChange={(e) => setSeedSnapshot(e.target.checked)} />
              使用所选账号已有快照
            </label>
            <button onClick={() => void createInstance()} disabled={creatingInstance || accounts.length === 0} className="btn-outline">
              <Plus size={14} /> {creatingInstance ? '创建中…' : '创建独立实例'}
            </button>
            <button onClick={() => void loadInstances()} disabled={loadingInstances} className="btn-ghost">
              <RefreshCw size={14} className={loadingInstances ? 'animate-spin' : ''} /> 刷新
            </button>
          </div>
          <div className="mt-3 space-y-2">
            {instances.length === 0 ? (
              <p className="rounded-md bg-slate-50 px-3 py-2 text-xs text-slate-400 dark:bg-zinc-900">暂无独立实例</p>
            ) : instances.map((item) => (
              <div key={item.id} className="flex flex-wrap items-center gap-2 rounded-md border border-slate-200 px-3 py-2 text-xs dark:border-zinc-700">
                <div className="min-w-0 flex-1">
                  <p className="truncate font-medium text-slate-700 dark:text-zinc-200">{item.name} <span className="font-mono text-slate-400">({item.id})</span></p>
                  <p className="truncate text-slate-400">账号 {item.account_uid} · {item.running ? `运行中${item.pid ? ` · PID ${item.pid}` : ''}` : '未运行'}</p>
                </div>
                {item.running ? (
                  <button onClick={() => void stopInstance(item.id)} disabled={instanceBusy === item.id} className="btn-outline !px-2 !py-1">
                    <Square size={12} /> {instanceBusy === item.id ? '处理中…' : '停止'}
                  </button>
                ) : (
                  <button onClick={() => void startInstance(item.id)} disabled={instanceBusy === item.id} className="btn-outline !px-2 !py-1">
                    <Play size={12} /> {instanceBusy === item.id ? '处理中…' : '启动'}
                  </button>
                )}
                <button onClick={() => void forgetInstance(item.id)} disabled={instanceBusy === item.id || item.running} className="btn-ghost !px-2 !py-1 text-rose-500">
                  移除登记
                </button>
              </div>
            ))}
          ...
          <h3 className="mb-1 font-medium">Trae Work 自动切换</h3>
          <p className="mb-3 text-xs text-slate-400">
            代理检测到文字对话或 Seedance 视频生成返回额度不足/鉴权失效时，自动选择账号池中可用积分最高的其他账号，保存当前登录态并重新打开项目。需保持代理运行；默认关闭。
          </p>
          <div className="space-y-3 text-sm">
            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={form.desktop_auto_switch}
                onChange={(e) => update('desktop_auto_switch', e.target.checked)}
              />
              启用额度耗尽后自动切换账号
            </label>
            <div className="flex items-center gap-2">
              <label className="label mb-0">触发冷却时间（秒）</label>
              <input
                type="number"
                value={form.desktop_auto_switch_cooldown_secs}
                onChange={(e) => update('desktop_auto_switch_cooldown_secs', Math.min(3600, Math.max(10, Number(e.target.value) || 60)))}
                className="input w-24"
                min={10}
                max={3600}
                disabled={!form.desktop_auto_switch}
              />
              <span className="text-xs text-slate-400">防止同一错误重复切号</span>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <button
                onClick={() => void runAutoSwitchSimulation()}
                disabled={simulatingAutoSwitch}
                className="btn-outline"
              >
                <PlayCircle size={15} /> {simulatingAutoSwitch ? '演练中…' : '模拟自动切换（不切号）'}
              </button>
              <span className="text-xs text-slate-400">只检查候选账号，不会访问上游或改变当前登录态</span>
            </div>
            {relayProgress.length > 0 && (
              <div className="rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-300">
                {relayProgress[relayProgress.length - 1]}
              </div>
            )}
          </div>
