...
export default function App() {
  ...
  // 全局积分自动刷新：无论当前停留在哪个页面，每 30 秒同步一次所有账号的剩余积分。
  // 使用 silent 模式避免后台刷新每 30 秒弹出成功/失败提示；手动刷新仍会提示结果。
  useEffect(() => {
    if (!ready) return;
    const timer = window.setInterval(() => {
      void refreshRemainingCredits({ silent: true });
    }, 30_000);
    return () => window.clearInterval(timer);
  }, [ready, refreshRemainingCredits]);
