...
export default function EcoAccess() {
  const [ccBusy, setCcBusy] = useState<'claude' | 'codex' | 'seedance' | null>(null);
  ...
  const registerSeedanceMcp = async () => {
    if (ccBusy) return;
    setCcBusy('seedance');
    setEcoNote('');
    try {
      const msg = await withMinDelay(api.apiServer.ccSwitchRegisterSeedance());
      setEcoNote(`✓ ${msg}`);
    } catch (e) {
      setEcoNote(`✗ Seedance MCP 注册失败：${String(e).slice(0, 160)}`);
    } finally {
      setCcBusy(null);
    }
  };
  ...
  return (
    <div className="card p-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button
          className="btn-outline flex items-center gap-1 !py-1.5 text-xs"
          onClick={() => void registerSeedanceMcp()}
          disabled={ccBusy !== null}
          title="把 Trae Work CN Seedance 注册为 Claude Code 的 MCP 工具；不作为文字模型 provider"
        >
          {ccBusy === 'seedance' ? '注册中…' : '注册 Seedance MCP（Claude Code）'}
        </button>
      </div>
      {ecoNote && (
      ...
      <p className="mt-2 text-[11px] text-slate-400 dark:text-zinc-500">
        ...
        写入后需重启 CC Switch 生效。Seedance 以 MCP 工具方式注册，使用同一网关 API Key；
        历史 Buddy 条目（aiwork-wb-gateway-*）不再使用，可手动在 CC Switch 中清理。
      </p>
    </div>
  );
}
