...
export default function GatewayHelpModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const base = `http://${gw?.listen_host ?? '127.0.0.1'}:${port}`;
    ...
    >
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-5">
        ...
        <div className="space-y-3 lg:col-span-2">
          ...
          <div className="rounded-lg border border-indigo-200 bg-indigo-50/60 p-3 text-xs dark:border-indigo-900/60 dark:bg-indigo-950/20">
            <p className="font-medium text-indigo-800 dark:text-indigo-200">Trae → DSH 生态接入</p>
            <p className="mt-1 leading-relaxed text-indigo-700/80 dark:text-indigo-300/80">
              可按本机网关地址配置 DSH 连接器，把 Trae 模型接入 DSH，并使用多账号切换与积分只读面板；本助手不复制或托管 DSH 凭证。
            </p>
            <p className="mt-1 text-[11px] text-indigo-600/70 dark:text-indigo-300/70">
              安装前请先关闭相关客户端，首次运行后按项目说明完成本机授权；所有凭证仍留在本机。
            </p>
          </div>
          ...
          <div className="space-y-1.5 text-xs">
            ...
            <Endpoint method="POST" path="/v1/videos/generations" note="Trae Work CN Seedance（Work 积分，异步任务）" />
            <Endpoint method="GET" path="/v1/videos/:task_id" note="查询 Seedance 任务状态与资源地址" />
            <Endpoint method="GET" path="/v1/videos/:task_id/content" note="下载网关缓存视频（API Key 隔离，失败时回退上游地址）" />
            <Endpoint method="POST" path="/v1/assets" note="上传参考图/视频（API Key 隔离）" />
            <Endpoint method="GET" path="/v1/assets/:asset_id/content?token=..." note="短时素材回取地址（显式配置后启用）" />
          ...
          <p className="text-[11px] leading-relaxed text-slate-400 dark:text-zinc-500">
            视频完成后网关会以流式方式缓存到 data/videos（可用 AIWORK_VIDEO_DIR 指定独立目录）；任务索引会在网关重启后恢复。
          </p>

          <p className="text-[11px] leading-relaxed text-slate-400 dark:text-zinc-500">
            参考图/视频可由 MCP 传本地路径或拖拽附件；网关只接收明确指定的单个文件。若要交给 Trae 云端读取，需在接口配置中设置 Trae 可访问的 HTTPS 素材基址。
          </p>
          ...
          <div className="rounded-lg bg-slate-50 p-3 text-xs dark:bg-zinc-800/50">
            ...
            <div className="space-y-1.5 leading-relaxed text-slate-500 dark:text-zinc-400">
              ...
              <p>
                ...
                Trae 池 / Buddy 池各自独立配置策略（默认积分先过期优先，可选 余额多优先 / 随机 / 三因子加权 / P2C / 固定顺序耗尽）。
              </p>
