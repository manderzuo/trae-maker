...
export default function ResourceSummary() {
  ...
  // Trae 池：同时汇总通用 / Work 余额；文字网关当前按通用积分取号，Work
  // 余额保留在统一资源画像中，后续资源级路由可直接复用。
  const traePoolCount = pool?.enabled_uids.length ?? 0;
  const traeWithCredits = accounts.filter((a) => (a.general_credits ?? 0) > 0 || (a.work_credits ?? 0) > 0).length;
  const traeWithGeneralCredits = accounts.filter((a) => (a.general_credits ?? 0) > 0).length;
  const traeWithWorkCredits = accounts.filter((a) => (a.work_credits ?? 0) > 0).length;
  const traeTotalCredits = accounts.reduce((s, a) => s + (a.general_credits ?? 0), 0);
  const traeWorkCredits = accounts.reduce((s, a) => s + (a.work_credits ?? 0), 0);
    ...
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
      ...
      <div className="card p-4">
        <div className="mb-3 flex items-center gap-2">
          ...
          <span className="ml-auto text-[11px] text-slate-400">通用 + Work 余额</span>
        </div>
        <div className="space-y-1.5">
          <Row label="池内账号" value={traePoolCount} />
          <Row label="含任一积分账号" value={traeWithCredits} />
          <Row label="含通用 / Work 账号" value={`${traeWithGeneralCredits} / ${traeWithWorkCredits}`} />
          <Row
            label="通用积分总余额"
          ...
          <Row
            label="Work 积分总余额"
            value={<span className="font-semibold text-sky-600 dark:text-sky-400">{fmtCredits(traeWorkCredits)}</span>}
          />
          <Row label="积分合计" value={fmtCredits(traeTotalCredits + traeWorkCredits)} />
          <Row
            label="池内调度策略"
