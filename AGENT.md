...
Windows 桌面端多账号签到 + 登录态切换 + 设备隔离 + API 网关一站式工作台，**深度支持 Trae Work 与 Trae（Trae CN IDE）双应用**（账号自动发现、切换/快照按目标应用独立、账号池 app 无关同池调度；桥档案表已预留豆包 / WorkBuddy）。数据目录默认使用 `%APPDATA%\AIWorkAssistant\`，也可由 `AIWORK_DATA_DIR` 指定可迁移目录；上游请求仅在用户启动对应功能时发出。
...
│   └── src/
│       ├── main.rs               # 注册全部命令
│       ├── state.rs              # AppState（AIWORK_DATA_DIR / %APPDATA%\AIWorkAssistant + python_dir + 旧目录迁移）
...
│   ├── seedance_mcp.py            # 零依赖 stdio MCP：CC/DSH 调用 Seedance 网关并轮询/下载
...
| OAuth | `oauth_login_bitbrowser(window_id, account_name?, group_id?)` → `OAuthLoginResult` | 在指定 BitBrowser profile 中完成 Trae 原生 PKCE 授权，回环接收 AuthCode、ExchangeToken 并加密入池；profile 仅作本次导航目标 |
...
| 环境 | `trae_instances_list()` / `trae_instance_create()` / `trae_instance_start()` / `trae_instance_stop()` / `trae_instance_forget()` | F-67 Trae Work 独立 `--user-data-dir` 实例管理；默认空目录，显式 `seed_snapshot` 才复制本机快照；移除登记保留目录 |
...
| API | `api_server_start()` / `api_server_stop()` / `api_server_status()` | API 网关启停（端口/监听地址/默认模型由设置页提供；LAN 监听强制 API Key；鉴权统一走 API Keys 列表） |
| API | `gateway_settings_get()` / `gateway_settings_set(settings)` | 网关监听端口、监听地址、默认模型与可选 CORS 来源 |
| API | `pool_list` / `pool_set` / `pool_status` | 账号池管理；`pool_set` 扩展 `strategy`（expire_first/credit_first/random/weighted/p2c/**sequential_drain**）/ `group_ids` / `wb_enabled`（T2.1 WB 上游开关）/ `wb_default_thinking`（T5.3）/ `wb_tool_exec`（T5.5，默认开）/ `wb_bg_downgrade`（T5.6③）——未传字段保留原值 |
...
| API | `api_conversations_list/export/settings_get/settings_set/delete` | 本机 API 会话列表、Markdown/JSON 导出、保留天数/正文持久化策略与单会话删除 |
| API | `gateway_tunnel_get/save/status/start/stop` | SSH 反向隧道参数、主机指纹校验、断线自动重连与显式启停；私钥只传路径 |
| API | `POST /v1/videos/generations` / `GET /v1/videos/:task_id` / `GET /v1/videos/:task_id/content` | Trae Work CN Seedance 文生视频异步任务桥；按 Work 积分取号、原生 SSE、幂等键、跨重启任务索引和受 Key 保护的视频分发 |
| MCP | `src-python/seedance_mcp.py` | 零依赖 stdio 工具 `seedance_generate`；只读取 `AIWORK_GATEWAY_BASE_URL` / `AIWORK_API_KEY`，不接触 Trae/BitBrowser 凭证 |
...
│   ├── api_pool.json             # API 账号池配置 + 状态（strategy 含 weighted/p2c/sequential_drain；wb_enabled 开关）
