...
pub struct AccountView {
    ...
    /// 登录态来源：bitbrowser / local / 手动导入等（仅用于选择续期通路）
    #[serde(default)]
...
pub struct RawAccount {
    ...
    /// 登录态来源。BitBrowser 导入账号续期时走 CDP 重捕获，不依赖本机快照。
...
pub struct Settings {
    ...
    /// BitBrowser 本地 API 地址（默认监听 127.0.0.1:54345）。
    /// 仅用于读取已登录的 Trae Work 页面，不保存密码、Cookie 或完整 JWT。
    #[serde(default = "default_bitbrowser_api_url")]
    ...
    /// Trae Work 桌面端遇到额度/鉴权失败时自动切换到账号池中的下一个账号（默认关闭）
    #[serde(default)]
    pub desktop_auto_switch: bool,
    /// 自动切号去抖冷却时间，避免同一失败请求重复触发
    #[serde(default = "default_desktop_auto_switch_cooldown")]
    pub desktop_auto_switch_cooldown_secs: u16,
}
...
fn default_desktop_auto_switch_cooldown() -> u16 {
    60
}

fn default_bitbrowser_api_url() -> String {
