...
/// 解析运行时数据根目录。
///
/// 桌面端保持 `%APPDATA%\AIWorkAssistant` 默认行为；服务器/便携部署可通过
/// `AIWORK_DATA_DIR` 指定绝对目录，旧的 `AIWORKDATA_DIR` 作为兼容别名。非
/// Windows 环境依次使用 `XDG_DATA_HOME`、`HOME/.local/share`，避免把运行位置
/// 写死在用户目录或仓库目录中。
pub fn resolve_data_dir() -> Result<PathBuf, String> {
    for name in ["AIWORK_DATA_DIR", "AIWORKDATA_DIR"] {
    ...
    if let Ok(home) = std::env::var("HOME") {
        if !home.trim().is_empty() {
            return Ok(PathBuf::from(home).join(".local").join("share").join(DATA_DIR_NAME));
        }
    }
    std::env::current_dir()
        .map(|dir| dir.join(".aiwork-data"))
        .map_err(|e| format!("无法解析 AI Work 数据目录: {e}"))
}
...
pub fn migrate_legacy_dirs() -> Option<String> {
    ...
    // 显式数据目录意味着便携/服务器部署：不把本机 APPDATA 的旧数据复制进来，
    // 避免服务器启动时意外接管桌面账号库。
    if std::env::var("AIWORK_DATA_DIR").ok().filter(|v| !v.trim().is_empty()).is_some()
        || std::env::var("AIWORKDATA_DIR").ok().filter(|v| !v.trim().is_empty()).is_some()
    {
        return None;
    }
...
/// 应用全局状态。base_dir 由 `resolve_data_dir` 决定，默认指向
...
impl AppState {
    pub fn new() -> Result<Self, String> {
        // 数据目录：AIWORK_DATA_DIR > AIWORKDATA_DIR > 平台默认目录
    ...
    pub fn settings(&self) -> Settings {
        ...
        // 新增桌面自动切号设置：旧配置文件没有该字段时 serde 默认为 0，
        // 回填安全的 60 秒去抖值，避免设置页显示 0 或事件失去防抖。
        if s.desktop_auto_switch_cooldown_secs == 0 {
            s.desktop_auto_switch_cooldown_secs = 60;
        }
        s
    }
}
