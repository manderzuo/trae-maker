...
pub fn classify_solo_error(code: i64, msg: &str) -> ErrKind {
    ...
    // 明确的积分耗尽与套餐限流分开：前者进入次日 04:00 保护，后者仍按 12h
    // PlanLimit 处理。只匹配“余额/积分不足”语义，避免把普通 rate/quota 错误误杀。
    if msg_lower.contains("insufficient credit")
        || msg_lower.contains("credits exhausted")
        || msg_lower.contains("credit exhausted")
        || msg_lower.contains("insufficient balance")
        || msg_lower.contains("积分不足")
        || msg_lower.contains("余额不足")
    {
...
mod error_classification_tests {
    ...
    #[test]
    fn explicit_credit_exhaustion_is_hard_credit() {
        assert_eq!(classify_solo_error(0, "insufficient credits"), ErrKind::HardCredit);
        assert_eq!(classify_solo_error(0, "积分不足，请切换账号"), ErrKind::HardCredit);
        assert_eq!(classify_solo_error(4008, "quota exceeded"), ErrKind::SoftRate);
    }
}
