...
pub enum PoolStrategy {
    ...
    P2C,
    /// 固定顺序耗尽：按 UID 稳定排序逐个使用，当前账号进入冷却后再到下一个。
    SequentialDrain,
}
...
impl PoolStrategy {
    ...
    pub fn parse(s: &str) -> Self {
        match s {
            ...
            "p2c" => Self::P2C,
            "sequential_drain" => Self::SequentialDrain,
            _ => Self::ExpireFirst,
        }
    }
...
impl ApiPool {
    ...
    pub fn sync_from_accounts(
        ...
        self.sync_from_accounts_with_balances(
            accounts,
            enabled_uids,
            group_ids,
            membership,
    ...
    /// 从账号文件同步双积分余额。`general_credits` 缺失时回退到旧的 total 缓存，
    /// 让旧版本 remaining_credits.json 可以无损升级。
    #[allow(clippy::too_many_arguments)]
    pub fn sync_from_accounts_with_balances(
        &self,
        accounts: &[crate::models::RawAccount],
        enabled_uids: &[String],
        group_ids: &[String],
        membership: &HashMap<String, String>,
        cooldowns: &HashMap<String, CooldownEntry>,
        total_credits: &HashMap<String, f64>,
        ...
        for a in accounts {
            if let Some(uid) = &a.user_id {
                ...
                let jwt_clean = jwt_raw
                    ...
                    .trim()
                    .to_string();
                let general = general_credits
                    .get(uid)
                    .copied()
                    .or_else(|| total_credits.get(uid).copied());
                let total = total_credits.get(uid).copied().or(general);
                let work = work_credits.get(uid).copied();
                entries.insert(
                    uid.clone(),
                    PoolEntry {
                        uid: uid.clone(),
    ...
    /// 运行中热更新账号余额。只更新本次刷新成功返回的 UID，避免单账号网络失败
    /// 把旧余额误清零；同时更新旧 `credits` 字段，因此现有取号/策略立即生效。
    pub fn update_credit_balances(
        &self,
        total_credits: &HashMap<String, f64>,
        general_credits: &HashMap<String, f64>,
        work_credits: &HashMap<String, f64>,
        ...
        for (uid, entry) in entries.iter_mut() {
            if !total_credits.contains_key(uid)
                ...
                && !work_credits.contains_key(uid)
            {
                continue;
            }
            let old = balances.get(uid).copied().unwrap_or_default();
            let general = general_credits.get(uid).copied().or(old.general);
            let total = total_credits.get(uid).copied().or_else(|| {
    ...
    pub fn pick_excluding_for(
        ...
        if kind == ResourceKind::General {
            return self.pick_excluding(tried);
        }
        let mut entries = safe_lock(&self.entries);
        ...
        if let Some(entry) = entries.get_mut(&account.uid) {
            entry.last_used = now;
        }
        Some(account)
    }
    ...
    pub fn status_list(&self) -> Vec<PoolStatus> {
        ...
        let mut out: Vec<PoolStatus> = entries
            ...
            .map(|e| PoolStatus {
                ...
                name: e.name.clone(),
                credits: e.credits,
                general_credits: balances.get(&e.uid).and_then(|b| b.general).or(e.credits),
                work_credits: balances.get(&e.uid).and_then(|b| b.work),
                total_credits: balances
                    .get(&e.uid)
                    .and_then(|b| b.total)
                    .or(e.credits),
...
fn selectable(e: &PoolEntry, tried: &HashSet<String>, now: i64) -> bool {
    selectable_with_credit(e, tried, now, e.credits)
}

fn selectable_with_credit(
    e: &PoolEntry,
    tried: &HashSet<String>,
    now: i64,
    credit: Option<f64>,
...
mod tests {
    ...
    fn strategy_parse_defaults_to_expire_first() {
        ...
        assert_eq!(PoolStrategy::parse("sequential_drain"), PoolStrategy::SequentialDrain);
    ...
    #[test]
    fn dual_credit_sync_and_hot_update_are_reflected_in_status() {
        let pool = ApiPool::new();
        let accounts = vec![acct("A", "uid_a"), acct("B", "uid_b")];
        let enabled = vec!["uid_a".to_string(), "uid_b".to_string()];
        let mut total = HashMap::new();
        total.insert("uid_a".to_string(), 120.0);
        total.insert("uid_b".to_string(), 80.0);
        let mut general = HashMap::new();
        general.insert("uid_a".to_string(), 100.0);
        general.insert("uid_b".to_string(), 0.0);
        let mut work = HashMap::new();
        work.insert("uid_a".to_string(), 20.0);
        work.insert("uid_b".to_string(), 80.0);
        ...
        let first = pool.status_list();
        let a = first.iter().find(|s| s.uid == "uid_a").unwrap();
        let b = first.iter().find(|s| s.uid == "uid_b").unwrap();
        assert_eq!(a.general_credits, Some(100.0));
        assert_eq!(a.work_credits, Some(20.0));
        assert_eq!(a.total_credits, Some(120.0));
        assert_eq!(b.general_credits, Some(0.0));
        assert_eq!(b.work_credits, Some(80.0));

        // 模拟 30s 刷新后：只更新成功返回的 UID，状态接口立即反映新余额。
