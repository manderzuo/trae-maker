...
pub fn stream_convert_lines(
    ...
    let (e, s, u, _) = stream_convert_lines_capture(lines, sender, chat_id);
    (e, s, u)
}

/// 与 `stream_convert_lines` 相同，同时返回已聚合的助手消息，供本地
/// 会话库记录。对旧调用保留三元组 API，避免影响现有消费者。
pub fn stream_convert_lines_capture(
    lines: Box<dyn Iterator<Item = String> + Send>,
    sender: tokio::sync::mpsc::Sender<Result<bytes::Bytes, std::io::Error>>,
    chat_id: &str,
) -> (Option<(i64, String)>, bool, Option<Value>, Option<Value>) {
    stream_convert_src_capture(lines, sender, chat_id)
}
...
fn stream_convert_src(
    ...
    let result = stream_convert_src_capture(src, sender, chat_id);
    (result.0, result.1, result.2)
}

fn stream_convert_src_capture(
    mut src: LineSrc,
    sender: tokio::sync::mpsc::Sender<Result<bytes::Bytes, std::io::Error>>,
    chat_id: &str,
) -> (Option<(i64, String)>, bool, Option<Value>, Option<Value>) {
    ...
    let mut captured_tool_calls: Vec<Value> = Vec::new();
                ...
                "output" | "thought" => {
                    ...
                    if let Some(tc) = &ev.tool_calls {
                        if let Some(arr) = tc.as_array() {
                            captured_tool_calls.extend(arr.iter().cloned());
    ...
    let assistant = if content.is_empty() && reasoning.is_empty() && captured_tool_calls.is_empty() {
        None
    } else {
        ...
        if !captured_tool_calls.is_empty() {
            message["tool_calls"] = json!(captured_tool_calls);
        }
        Some(message)
    };
    (
...
pub fn stream_convert_text_lines(
    ...
    let (e, s, u, _) = stream_convert_text_lines_capture(lines, sender, completion_id, model);
    (e, s, u)
}

pub fn stream_convert_text_lines_capture(
    lines: Box<dyn Iterator<Item = String> + Send>,
    sender: tokio::sync::mpsc::Sender<Result<bytes::Bytes, std::io::Error>>,
    completion_id: &str,
    model: &str,
) -> (Option<(i64, String)>, bool, Option<Value>, Option<Value>) {
    stream_convert_text_src_capture(lines, sender, completion_id, model)
}
...
fn stream_convert_text_src(
    ...
    let result = stream_convert_text_src_capture(src, sender, completion_id, model);
    (result.0, result.1, result.2)
}

fn stream_convert_text_src_capture(
    mut src: LineSrc,
    sender: tokio::sync::mpsc::Sender<Result<bytes::Bytes, std::io::Error>>,
    completion_id: &str,
    model: &str,
    ...
    (error_info, sent_any, pending_usage, assistant)
}
...
pub fn stream_convert_anthropic_lines(
    ...
    let (e, s, u, _) = stream_convert_anthropic_lines_capture(lines, sender, msg_id, model);
    (e, s, u)
}

pub fn stream_convert_anthropic_lines_capture(
    lines: Box<dyn Iterator<Item = String> + Send>,
    sender: tokio::sync::mpsc::Sender<Result<bytes::Bytes, std::io::Error>>,
    msg_id: &str,
    model: &str,
) -> (Option<(i64, String)>, bool, Option<Value>, Option<Value>) {
    stream_convert_anthropic_src_capture(lines, sender, msg_id, model)
}
...
fn stream_convert_anthropic_src(
    ...
    let result = stream_convert_anthropic_src_capture(src, sender, msg_id, model);
    (result.0, result.1, result.2)
}

fn stream_convert_anthropic_src_capture(
    mut src: LineSrc,
    sender: tokio::sync::mpsc::Sender<Result<bytes::Bytes, std::io::Error>>,
    msg_id: &str,
    model: &str,
    ...
    while let Some(line) = src.next() {
        if let Some(ev) = scan_line(&mut st, &line.trim_end()) {
            match ev.event.as_str() {
                "output" | "thought" => {
                    captured_content.push_str(&ev.response);
                    if !ev.response.is_empty() {
                        start_message!();
                        if !text_block_open {
                            open_text_block!();
    ...
    let assistant = if captured_content.is_empty() && tools.is_empty() {
        None
    } else {
        let mut message = json!({"role": "assistant", "content": captured_content});
        if !tools.is_empty() {
            let calls: Vec<Value> = tools.iter().enumerate().map(|(i, t)| {
                json!({
                    "id": if t.id.is_empty() { format!("toolu_{}_{}", msg_id, i) } else { t.id.clone() },
                    "type": "function",
...
/// SOLO SSE → Codex Responses SSE。F-41/Trae2Codex 使用：SOLO 的事件格式与
/// WorkBuddy 不同，不能直接复用 wb_sse 的 OpenAI parser；这里保留同一套
/// response.created / output_text.delta / response.completed 事件语义。
pub fn stream_convert_responses_lines_capture(
    mut lines: Box<dyn Iterator<Item = String> + Send>,
    sender: tokio::sync::mpsc::Sender<Result<bytes::Bytes, std::io::Error>>,
    response_id: &str,
    model: &str,
) -> (Option<(i64, String)>, bool, Option<Value>, Option<Value>) {
    ...
    let mut usage: Option<Value> = None;
    let mut error_info: Option<(i64, String)> = None;
    let send_event = |event: &str, data: Value| {
        let _ = sender.blocking_send(Ok(bytes::Bytes::from(format!(
            "event: {event}\ndata: {data}\n\n"
        ))));
    };
    let mut ensure_created = |created: &mut bool, sent_any: &mut bool| {
        if !*created {
            *created = true;
            *sent_any = true;
            send_event("response.created", json!({
                "type": "response.created",
                "response": {"id": response_id, "object": "response", "created_at": now_ts(), "status": "in_progress", "model": model, "output": [], "error": null}
            }));
            send_event("response.in_progress", json!({
    ...
    while let Some(line) = lines.next() {
        if let Some(ev) = scan_line(&mut state, &line.trim_end()) {
            match ev.event.as_str() {
                "output" | "thought" => {
                    ensure_created(&mut created, &mut sent_any);
                    if !ev.response.is_empty() {
                        content.push_str(&ev.response);
                        send_event("response.output_text.delta", json!({
                            "type": "response.output_text.delta", "delta": ev.response,
                            "item_id": format!("msg_{response_id}"), "output_index": 0, "content_index": 0,
                        }));
                    }
                    if !ev.reasoning.is_empty() { reasoning.push_str(&ev.reasoning); }
                }
                "token_usage" => usage = ev.usage,
                "done" | "turn_completion" => {
                    ensure_created(&mut created, &mut sent_any);
                    finish_reason = if ev.finish_reason.is_empty() { "stop".into() } else { ev.finish_reason };
                }
                "error" => {
                    let code = ev.error_code.unwrap_or(0);
                    let msg = if ev.error_message.is_empty() { "upstream error".into() } else { ev.error_message };
                    if created {
                        send_event("response.failed", json!({
                            "type": "response.failed", "response": {"id": response_id, "status": "failed", "error": {"code": code, "message": msg}}
                        }));
                        error_info = Some((code, msg));
                    } else { error_info = Some((code, msg)); }
                }
                _ => {}
            }
        }
    }
    if let Some(err) = error_info.clone() {
        if !created { return (Some(err), false, usage, None); }
        return (None, true, usage, None);
    }
    ensure_created(&mut created, &mut sent_any);
    if !content.is_empty() {
        send_event("response.output_text.done", json!({
            "type": "response.output_text.done", "text": content,
            "item_id": format!("msg_{response_id}"), "output_index": 0, "content_index": 0,
        }));
    }
    send_event("response.output_item.done", json!({
        "type": "response.output_item.done",
        "item": {"id": format!("msg_{response_id}"), "type": "message", "status": "completed", "role": "assistant",
                  "content": if content.is_empty() { json!([]) } else { json!([{"type":"output_text","text":content,"annotations":[]}]) }},
        "output_index": 0,
    }));
    let (input_tokens, output_tokens) = usage.as_ref().map(extract_tokens).unwrap_or((0, 0));
    send_event("response.completed", json!({
        "type": "response.completed",
        "response": {"id": response_id, "object": "response", "created_at": now_ts(),
          "status": if finish_reason == "length" { "incomplete" } else { "completed" },
          "model": model,
          "output": if content.is_empty() { json!([]) } else { json!([{"id":format!("msg_{response_id}"),"type":"message","status":"completed","role":"assistant","content":[{"type":"output_text","text":content,"annotations":[]}]}]) },
          "usage": {"input_tokens": input_tokens, "output_tokens": output_tokens, "total_tokens": input_tokens + output_tokens}, "error": null}
    }));
    let assistant = if content.is_empty() { None } else { Some(json!({"role":"assistant","content":content})) };
    let _ = reasoning; // 当前 Responses 投影不伪造独立 reasoning block。
    (None, sent_any, usage, assistant)
}
...
mod tests {
    ...
    #[test]
    fn capture_entry_returns_assistant_message_without_changing_sse() {
        let input = "event: output\ndata: {\"response\":\"hello\"}\n\nevent: output\ndata: {\"response\":\" world\"}\n\nevent: done\ndata: {\"finish_reason\":\"stop\"}\n\n";
        let (tx, rx) = tokio::sync::mpsc::channel::<Result<bytes::Bytes, std::io::Error>>(64);
        let (error, sent, _usage, assistant) = stream_convert_lines_capture(
            Box::new(input.lines().map(str::to_string)),
            tx,
            "c1",
        );
        ...
        assert!(sent);
        assert_eq!(assistant.unwrap()["content"], "hello world");
        assert!(collect_events(rx).join("\n").contains("hello"));
    }

    #[test]
    fn responses_capture_emits_standard_lifecycle_and_assistant() {
        let input = "event: output\ndata: {\"response\":\"hello\"}\n\n\
                     event: output\ndata: {\"response\":\" world\"}\n\n\
                     event: token_usage\ndata: {\"usage\":{\"prompt_tokens\":2,\"completion_tokens\":3}}\n\n\
                     event: done\ndata: {\"finish_reason\":\"stop\"}\n\n";
        let (tx, rx) = tokio::sync::mpsc::channel::<Result<bytes::Bytes, std::io::Error>>(64);
        let (error, sent, _usage, assistant) = stream_convert_responses_lines_capture(
            Box::new(input.lines().map(str::to_string)), tx, "resp_1", "seedance-text",
        );
        assert!(error.is_none());
        assert!(sent);
        assert_eq!(assistant.unwrap()["content"], "hello world");
        let events = collect_events(rx).join("\n");
