...
pub fn prepare_llm_chat_body(
    ...
    prepare_llm_chat_body_with_conversation(src, default_model, uid, device_id, machine_id, None)
}

/// 带本地会话键的 Trae 上游请求改写。
/// `conversation_id` 是网关本地会话键；真正发往上游的 ID 会按 UID 做
/// 命名空间隔离，账号切换后不会复用另一账号的云端会话。
pub fn prepare_llm_chat_body_with_conversation(
    src: &[u8],
    default_model: &str,
    uid: &str,
    device_id: &str,
    ...
    let upstream_conversation_id = conversation_id
        .filter(|s| !s.trim().is_empty())
        .map(|s| super::conversation::scoped_upstream_id(s, uid, "conversation"))
        .unwrap_or_else(gen_uuid_like);
    obj_mut.insert("conversation_id".into(), json!(upstream_conversation_id));
    obj_mut.insert("user_id".into(), json!(uid));
    let upstream_session_id = conversation_id
        .filter(|s| !s.trim().is_empty())
        .map(|s| super::conversation::scoped_upstream_id(s, uid, "session"))
        .unwrap_or_else(gen_uuid_like);
    obj_mut.insert("session_id".into(), json!(upstream_session_id));
...
mod tests {
    ...
    #[test]
    fn local_conversation_uses_account_scoped_stable_upstream_ids() {
        let src = json!({
            "model": "glm-5.2",
        ...
        let a: Value = serde_json::from_slice(&prepare_llm_chat_body_with_conversation(
            &bytes, "deepseek-v4-flash", "uid-a", "dev-a", "machine-a", Some("local-session-1"),
        )).unwrap();
        let a2: Value = serde_json::from_slice(&prepare_llm_chat_body_with_conversation(
            &bytes, "deepseek-v4-flash", "uid-a", "dev-a", "machine-a", Some("local-session-1"),
        )).unwrap();
        let b: Value = serde_json::from_slice(&prepare_llm_chat_body_with_conversation(
            &bytes, "deepseek-v4-flash", "uid-b", "dev-b", "machine-b", Some("local-session-1"),
        )).unwrap();
