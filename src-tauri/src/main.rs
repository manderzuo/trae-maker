...
fn main() {
    ...
    let app = builder
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::instances::trae_instances_list,
            commands::instances::trae_instance_create,
            commands::instances::trae_instance_start,
            commands::instances::trae_instance_stop,
            commands::instances::trae_instance_forget,
            ...
            commands::proxy::proxy_auto_switch_simulate,
            ...
            commands::api_server::api_conversations_list,
            commands::api_server::api_conversations_export,
            commands::api_server::api_conversations_settings_get,
            commands::api_server::api_conversations_settings_set,
            commands::api_server::api_conversations_delete,
            commands::api_server::api_keys_list,
            commands::api_server::api_keys_usage,
            commands::api_server::api_keys_save,
            commands::api_server::bridge_key_status,
            commands::api_server::bridge_key_issue,
            commands::api_server::bridge_key_revoke,
            ...
            commands::tunnel::gateway_tunnel_get,
            commands::tunnel::gateway_tunnel_save,
            commands::tunnel::gateway_tunnel_status,
            commands::tunnel::gateway_tunnel_start,
            commands::tunnel::gateway_tunnel_stop,
            commands::frp::gateway_frp_get,
            commands::frp::gateway_frp_save,
            commands::frp::gateway_frp_status,
            commands::frp::gateway_frp_start,
            commands::frp::gateway_frp_stop,
            ...
            commands::ccswitch::ccswitch_register_seedance_mcp,
            ...
            commands::relay::trae_relay_capture,
            commands::relay::trae_relay_create_share,
            commands::relay::trae_relay_open_share,
            commands::relay::open_trae_project,
            ...
            commands::oauth::oauth_poll_callback,
            commands::oauth::oauth_parse_callback,
            commands::oauth::oauth_login,
            commands::oauth::oauth_login_bitbrowser,
            commands::trae_apps::apps_accounts_discover,
            commands::trae_apps::apps_account_add,
            commands::bitbrowser::bitbrowser_accounts_discover,
            commands::bitbrowser::bitbrowser_account_import,
            commands::bitbrowser::bitbrowser_account_renew,
            commands::bitbrowser::bitbrowser_profiles_list,
        ...
        .setup(|app| {
            ...
            // 可选的启动期 API 网关：便携启动器设置 AIWORK_AUTO_START_API=1 时，
            // 在托盘/窗口初始化后自动恢复网关，避免重启后 FRP 已连接但 7864 尚未监听。
            // 默认不启用，保持手动启动语义；失败只写日志，不阻断桌面端启动。
            if std::env::var("AIWORK_AUTO_START_API").ok().as_deref() == Some("1") {
                ...
                std::thread::spawn(move || {
                    ...
                    let runtime = app2
                        .state::<Mutex<Option<commands::api_server::ApiServerRuntime>>>();
                    let result = tauri::async_runtime::block_on(commands::api_server::do_start(
                        &app2, &st, &runtime,
                    ));
                    match result {
                        Ok(status) => fs_utils::app_log(
                            &st.data_dir,
                            &format!("启动器自动启动 API 网关成功: {}:{}", status.host, status.port),
                        ),
                        Err(error) => fs_utils::app_log(
                            &st.data_dir,
                            &format!("启动器自动启动 API 网关失败: {error}"),
                        ),
                    }
                });
            }
