...
use crate::commands::api_server::ApiServerRuntime;
    ...
    let merged: Vec<serde_json::Value> = views
        ...
        .map(|v| {
            ...
            serde_json::json!({
                ...
                "generalCredits": v.general_credits,
                "workCredits": v.work_credits,
                "totalCredits": v.remaining_credits,
...
pub fn accounts_import(
    ...
    let mut device_map: DeviceMap = fs_utils::read_json(&state.path("device_map.json"));
    let mut credits: RemainingCreditsFile =
        fs_utils::read_json(&state.path("remaining_credits.json"));
    let mut imported_local_state = false;
    ...
    for (_, entry) in selected {
        ...
        // 迁移到新电脑时保留已知 device_id；缺失时网关仍会按 uid 确定性派生。
        if let Some(device_id) = pick_str(entry, &["deviceId", "device_id"]) {
            if !device_map.contains_key(&uid) {
                device_map.insert(
                    uid.clone(),
                    crate::models::DeviceEntry {
                        device_id,
                        market_user_id: None,
                        session_id: None,
                    },
                );
                imported_local_state = true;
            }
        }
        // 余额仅作为导入后的初始展示值，下一次 30 秒刷新会以服务端实时值覆盖。
        if let Some(value) = pick_f64(entry, &["remainingCredits", "totalCredits", "balance"]) {
            credits.credits.insert(uid.clone(), value.max(0.0));
            imported_local_state = true;
        }
        if let Some(value) = pick_f64(entry, &["generalCredits", "general_credits"]) {
            credits.general.insert(uid.clone(), value.max(0.0));
            imported_local_state = true;
        }
        if let Some(value) = pick_f64(entry, &["workCredits", "work_credits"]) {
            credits.work.insert(uid.clone(), value.max(0.0));
            imported_local_state = true;
        }
        if let Some(value) = pick_i64(entry, &["creditsExpireAt", "credits_expire_at"]) {
            credits.expire_times.insert(uid.clone(), value);
            imported_local_state = true;
        }
        accounts.accounts.push(RawAccount {
            ...
            credential_source: pick_str(entry, &["credentialSource", "credential_source"]),
        });
        report.added += 1;
    }
    ...
    if report.added > 0 || report.groups_added > 0 {
        ...
        if imported_local_state {
            fs_utils::write_json(&state.path("device_map.json"), &device_map)?;
            credits.updated_at = Some(fs_utils::now_iso());
            fs_utils::write_json(&state.path("remaining_credits.json"), &credits)?;
        }
        fs_utils::app_log(
            &state.data_dir,
            &format!(
...
/// async 派发：内部走网络请求（最长 120s），同步命令跑主线程会冻住 UI。
#[tauri::command(async)]
pub fn fetch_remaining_credits(
    state: State<AppState>,
    ...
    if let Some(exp) = stats.earliest_expire {
        ...
        rc.expire_times.remove(&user_id);
    }
    ...
    if let Some(rt) = runtime.lock().unwrap_or_else(|e| e.into_inner()).as_ref() {
        rt.shared.pool.update_credit_balances(
            &rc.credits,
            &rc.general,
            &rc.work,
            &rc.expire_times,
...
pub fn refresh_remaining_credits(
    ...
    for a in &accounts.accounts {
        ...
        // 访问令牌是短期 JWT（通常约 8 小时），refresh_token 才是长期续期凭据。
        // 积分定时刷新遇到 401 时自动做一次本机 ExchangeToken，再用新 JWT 重试，
        // 避免账号因 access token 到期被持续标记为失败。
        let mut stats_result = calc_remaining_credits(&a.jwt, &dev);
        if let Err(error) = &stats_result {
            let can_refresh = a
                .refresh_token
                .as_deref()
                .is_some_and(|token| !token.trim().is_empty());
            if can_refresh && error.contains("status code 401") {
                match refresh_jwt_impl(&state, &uid) {
                    Ok(new_jwt) => {
                        fs_utils::app_log(
                            &state.data_dir,
                            &format!("积分刷新遇到 401，已自动续期 JWT: uid={}", uid),
                        );
                        stats_result = calc_remaining_credits(&new_jwt, &dev);
                    }
                    Err(refresh_error) => {
                        fs_utils::app_log(
                            &state.data_dir,
                            &format!("积分刷新自动续期失败: uid={} error={refresh_error}", uid),
                        );
                    }
                }
            }
    ...
    // 与磁盘缓存同一批次热同步运行中的 SOLO 账号池：不需要重启网关，
    // 下一次取号即可看到最新的通用/Work/总余额。
    if let Some(rt) = runtime.lock().unwrap_or_else(|e| e.into_inner()).as_ref() {
        rt.shared.pool.update_credit_balances(
            &rc.credits,
            &rc.general,
...
pub fn refresh_jwt_impl(state: &AppState, user_id: &str) -> Result<String, String> {
    ...
    // 兼容不同 CloudIDE 网关版本：code 可能是数字、数字字符串，或只在
    // data/Result 信封中返回；只要响应里有 access_token，就按成功处理。
    let code = body
        .get("code")
        .and_then(|value| {
            value
    ...
    if code != 0 {
        ...
        crate::fs_utils::app_log(
            &state.data_dir,
            &format!("ExchangeToken 返回非成功响应: uid={} code={} keys=[{}]", user_id, code, keys),
        );
        return Err(format!("ExchangeToken 失败 (code={}): {}", code, msg));
    }

    // F-49 宽容解析：data 信封内字段直接 dig 查找，兼容嵌套包裹
    let new_access_token = crate::fs_utils::dig(
        &body,
        &[
            "access_token",
            "token",
    ...
    // 原子写回
    let (log_name, effective_refresh_token) = {
        let account = accounts
            .accounts
            .iter_mut()
            .find(|a| a.user_id.as_deref() == Some(user_id))
        ...
        (account.name.clone(), account.refresh_token.clone())
    };
    crate::vault::save_accounts(&state, &mut accounts)?;

    // 原生续期成功后同步更新 icube 快照。BitBrowser 只承担首次注册/登录；
    // 后续 JWT/refresh_token 续期完全走本地 ExchangeToken，不再依赖 BitBrowser 窗口。
    if let Err(error) = crate::native_snapshot::ensure_native_snapshot(
        ...
        fs_utils::app_log(
            &state.data_dir,
            &format!("原生续期后同步快照失败: uid={} error={error}", user_id),
        );
    }
...
pub fn build_account_views(state: &AppState) -> Vec<AccountView> {
    ...
    let summary_today = summary
        ...
        .unwrap_or(false);
    // 签到结果以 UID 为主键；旧版 summary 可能只有 name，才使用名称兜底。
    // 不能把所有结果统一按名称匹配，否则改名或同名账号会被误标为「已签」。
    let mut checked_uids = std::collections::HashSet::new();
    let mut checked_legacy_names = std::collections::HashSet::new();
    if summary_today {
        for result in &summary.results {
            let ok = result.get("ok").and_then(|v| v.as_bool()).unwrap_or(false);
            let action = result.get("action").and_then(|v| v.as_str()).unwrap_or("");
            if !(ok || (action != "fail" && !action.is_empty())) {
                continue;
            }
            ...
            {
                checked_uids.insert(uid.to_string());
            } else if let Some(name) = result
                .get("name")
                .and_then(|v| v.as_str())
                .filter(|name| !name.trim().is_empty())
            {
                checked_legacy_names.insert(name.to_string());
            }
        }
    }
    // Rust 侧最终结果文件始终按 UID 记录；即便旧 summary 被覆盖/损坏，也能保留当日状态。
    let today_key = crate::checkin_results::today_key();
    if let Some(day) = crate::checkin_results::load(&state.data_dir)
        .days
        .get(&today_key)
    {
        checked_uids.extend(day.accounts.iter().filter_map(|(uid, result)| {
            (result.status == "success" || result.status == "already").then(|| uid.clone())
        }));
    }
