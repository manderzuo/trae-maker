...
pub fn switch_account(
    ...
    let is_trae_work = matches!(target_app.as_deref(), None | Some("TraeWork"));
    ...
    // 兼容旧版 BitBrowser 导入账号，并修复“短 JWT 覆盖快照”的历史状态：
    // 切换 Work 前优先用账号池里的 refresh_token 换出新 JWT，再把 JWT +
    // refresh_token 同步进原生快照。这样即使 BitBrowser 页面捕获的是临近过期
    // 的旧 JWT，切换也不会把失效登录态恢复到 TRAE SOLO CN。
    if is_trae_work {
        let snapshot = state.path("profiles").join(user_id.trim());
        let accounts = crate::vault::load_accounts(&state);
        let account = accounts
            .accounts
            .iter()
            .find(|account| account.user_id.as_deref() == Some(user_id.trim()));
        if let Some(account) = account {
            let has_refresh_token = account
                .refresh_token
                .as_deref()
                .is_some_and(|token| !token.trim().is_empty());
            let near_expiry = crate::jwt::parse(&account.jwt)
                .exp_timestamp
                .map(|exp| exp <= chrono::Utc::now().timestamp() + 120)
                .unwrap_or(true);
            let mut refreshed = false;
            if has_refresh_token && near_expiry {
                match crate::commands::accounts::refresh_jwt_impl(&state, user_id.trim()) {
                    Ok(_) => {
                        refreshed = true;
                        fs_utils::app_log(
                            &state.data_dir,
                            &format!("切换前原生 JWT 自动刷新成功: uid={}", user_id),
                        );
                    }
                    Err(error) => fs_utils::app_log(
                        &state.data_dir,
                        &format!("切换前原生 JWT 自动刷新失败: uid={} error={error}", user_id),
                    ),
                }
            }

            // 刷新后重新读取账号，拿到可能轮换的新 refresh_token；即使快照已经
            // 存在，只要账号有 refresh_token 也重新同步一次，修复旧版空字段快照。
            let latest = crate::vault::load_accounts(&state);
            let latest_account = latest
                .accounts
                .iter()
                .find(|candidate| candidate.user_id.as_deref() == Some(user_id.trim()))
                .unwrap_or(account);
            if (!snapshot.is_dir() || has_refresh_token || refreshed)
                && !latest_account.jwt.trim().is_empty()
            {
                match crate::native_snapshot::ensure_native_snapshot(
                    &state,
                    user_id.trim(),
                    &latest_account.jwt,
                    latest_account.refresh_token.as_deref(),
                    &latest_account.name,
                ) {
                    Ok(outcome) => fs_utils::app_log(
                        &state.data_dir,
                        &format!(
                            "切换前同步原生快照: uid={} created={} updated={}",
                            user_id, outcome.created, outcome.updated
                        ),
                    ),
                    Err(error) => fs_utils::app_log(
                        &state.data_dir,
                        &format!("切换前同步原生快照失败: uid={} error={error}", user_id),
                    ),
                }
            }
        }
