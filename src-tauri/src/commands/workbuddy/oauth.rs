...
//!
//! 流程（无 PKCE，state 服务端签发；对齐已验证的官方客户端行为）：
...
/// 宽容解析 auth/state 响应中的登录 URL（F-50，对齐本地实测响应）：
...
fn oauth_flow(app: &AppHandle, state: &AppState) -> Result<(String, String), String> {
    // API 上游在 copilot.tencent.com——打到
    ...
    // login/account 请求带 Bearer；不带头会被当作未认证请求拿不到资料
...
mod oauth_tests {
    ...
    ///    （服务端返回的 authUrl 不追加参数）；data 信封包裹同样可解析
    #[test]
