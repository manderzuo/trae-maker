...
fn storage_uid_evidence(storage: &serde_json::Value) -> HashMap<String, UidEvidence> {
    ...
    // F-70：直接读取原生 tc 登录容器。只把 Cloud-IDE uid 作为本机证据，
    // 不把解密后的 token 返回前端，也不写入日志；失败时继续使用旧证据链。
    if let Some(encoded) = storage
        .get("iCubeAuthInfo://icube.cloudide")
        .and_then(|value| value.as_str())
    {
        if let Ok(plain) = crate::native_snapshot::decrypt_tc(encoded) {
            if let Ok(auth) = serde_json::from_str::<serde_json::Value>(&plain) {
                if let Some(uid) = auth
                    .get("userId")
                    .and_then(|value| value.as_str())
                    .filter(|uid| is_uid_token(uid))
                {
                    let entry = out.entry(uid.to_string()).or_default();
                    entry.count += 8;
                }
            }
        }
...
pub struct DiscoveredAccount {
    ...
    /// 发现来源：local / bitbrowser
    #[serde(default)]
    pub source: String,
    /// BitBrowser profile 标识（source=bitbrowser 时填写）
    #[serde(default)]
    pub window_id: Option<String>,
    #[serde(default)]
    pub window_seq: Option<i64>,
    #[serde(default)]
    pub window_name: Option<String>,
    #[serde(default)]
    pub page_url: Option<String>,
    /// 页面 localStorage 是否存在 Cloud-IDE-Token
    #[serde(default)]
...
/// 供 BitBrowser 等外部登录态发现器复用账号池 uid 去重规则（不暴露给前端）。
pub(crate) fn pool_uid_set_for_discovery(
    accounts: &crate::models::AccountsFile,
) -> std::collections::HashSet<String> {
    pool_uid_set(accounts)
}
