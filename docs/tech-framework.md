...
本节只记录本项目自身的实现边界与验证依据，不嵌入第三方仓库、作者或个人链接。

| 能力方向 | 本项目价值落点 |
|---|---|
| 上游适配与调度 | 账号池取号、请求改写、会话粘性与失败回退 |
| 多协议网关 | OpenAI / Anthropic / Responses / MCP 兼容输出 |
| 多实例与账号隔离 | 独立数据目录、快照校验、设备标识隔离与恢复 |
| 多模态任务桥 | Seedance 视频、图片/视频素材上传、异步轮询与缓存 |
| 可靠性与安全 | 限流、重试、凭证脱敏、URL 校验、健康检查与审计日志 |
