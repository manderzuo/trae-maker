...
> **版本**: v3.2.8 | **平台**: Windows 10 / 11 | **更新日期**: 2026-09-16
...
- **API 管理**（钥匙图标）— 全局网关管理弹窗：服务启停 / 接口配置 / API Keys / 生态接入 / 用量统计（见第 7 章）
...
支持三种方式添加账号（OAuth 另提供 BitBrowser 原生接管入口）：
...
> OAuth 登录的账号自带 refresh_token，支持 JWT 自动续期。JWT 通常约 8 小时，refresh_token 用于长期续期。

**BitBrowser 原生接管（推荐用于临时/无痕窗口）**：在 OAuth 弹窗的 BitBrowser 区域选择窗口并点击「在 BitBrowser 中开始登录」。助手会把 Trae 原生授权页导航到该窗口，自动接收回调、完成 PKCE/AuthCode ExchangeToken，并将短期 JWT、长期 refresh_token 与原生切换快照加密保存到账号库。首次导入完成后临时 profile 可以删除，后续刷新/续期直接走本机原生 Trae Work CN，不再依赖 BitBrowser；授权期间需要保持助手运行，登录/验证码等交互仍由用户在 BitBrowser 中完成。
...
| **续期** (KeyRound 图标) | 使用账号的原生 refresh_token 通过 Trae Work CN ExchangeToken 续期，并同步快照；旧账号无 refresh_token 时需从 BitBrowser 重新导入一次 |
...
> **BitBrowser 账号说明**：BitBrowser 只用于首次注册/登录和一次性接管原生 refresh_token。首次导入完成后账号池保存的是加密凭据与原生 TRAE Work CN 快照，不绑定窗口 ID；临时 profile 可以删除，后续续期不需要重新打开 BitBrowser。旧版本导入的账号若没有 refresh_token，再从 BitBrowser 重新导入一次即可升级。
...
在「系统设置 → 环境配置 → Trae Work 独立实例」可以为账号创建独立实例。每个实例使用单独的 `--user-data-dir`，因此可在资源允许时并行运行多个 Trae Work 窗口。

- 填写唯一实例 ID，选择账号后点击「创建独立实例」；默认创建空目录，首次启动后需在该窗口内正常登录。
- 只有明确勾选「使用所选账号已有快照」时，才会把本机快照复制到新目录；不会上传凭证，也不会自动绕过登录验证。
- 「启动」会复用当前代理端口（若代理正在运行）；「停止」只结束该实例进程；「移除登记」只移除注册表项并保留数据目录。
- 实例目录隔离不等于服务端设备绑定解除。若 Trae 限制单进程或动态设备头不兼容，请使用原有账号切换流程。
...
点击左侧栏 **API 管理** 图标（钥匙图标）打开弹窗，任意页面可用，Tab 分区：
...
- **积分体系**：Trae 通用积分（product_id 208，签到 / 活动获取），各模型按倍率消耗；账号池轮换消耗各账号积分，冷却与禁用自动处理（可一键清除冷却）
- **池指标**：可用账号数（健康且未冷却）/ 活跃账号（当日被调度使用）/ 池内账号
- **账号池选择**：勾选参与调度的账号，配置调度策略与分组筛选；账号行同时显示通用积分、Work 积分和合计余额。保存后需重启服务应用成员变更，余额刷新可热更新。调度时自动跳过禁用 / 冷却中 / 积分已过期的账号，单账号额度耗尽自动换号并可继续尝试池内后续账号。
- **账号池迁移**：资源调度页提供「导出账号池 / 导入账号池」，会迁移池成员、分组、策略、设备映射、初始余额及对应账号凭据。迁移 JSON 等同密码，建议仅通过加密 U 盘或安全传输，导入验证后立即删除。
...
- **积分体系**：Buddy 积分（签到 / 成长中心获取），按模型倍率消耗，与 Trae 通用积分相互独立
- **池指标**：健康账号（含凭证且无需重新登录）/ 今日使用 / 池内账号
- **资源开关**：启用 WB 上游（总开关）/ 默认深度思考 / 工具执行 / 后台任务降级
- **账号池选择**：含凭证账号参与 WB 上游调度（粘性会话 / 模型级冷却自动处理）
- **模型目录（Buddy）**：「同步目录」拉取上游目录，字段同上

### 7.4 使用方式

网关启动后，可使用标准 OpenAI SDK 调用（`api_key` 使用全局 API 管理「API Keys 管理」中创建的 Key；未配置启用 Key 时不鉴权）：
...
- `POST /v1/assets` — 上传参考图/参考视频（JSON Base64，API Key 隔离，单文件最多 32 MiB）
- `GET /v1/assets/:asset_id/content?token=...` — 仅在显式公网素材基址配置后生成的短时素材链接
- `POST /v1/videos/generations` — Trae Work CN Seedance 异步视频任务（按 Work 积分取号）
...
默认请求仍是无状态的。需要在账号 A 用尽后由账号 B 接着上下文时，给每轮请求
传入同一个会话标识即可：

```http
X-Conversation-ID: project-demo-001
```

或在 JSON 顶层使用 `conversation_id` 字段。首次成功响应后，助手会把规范化的
消息保存在本机 `data/conversations.sqlite3`；后续请求只提交最新一条消息时，网关
会自动补齐历史，客户端若提交完整 `messages` 则按重叠部分去重。数据库只在显式
启用会话标识时创建，旧客户端不传标识不会改变行为。

为避免跨账号复用 Trae 云端会话，实际发往上游的 `conversation_id`/`session_id`
会按账号 UID 分区；本地会话键保持不变，因此账号池换号后仍能继续上下文。会话正文
仅存本机，不写入 API 请求日志。API 配置页提供会话列表、Markdown/JSON 导出、删除、保留天数和正文持久化开关。
补充；如需立即清除，可停止助手后删除该 SQLite 文件。
...
### 8.5 Trae Work 桌面端自动切换

在「环境配置 → Trae Work 自动切换」开启后，代理仅观察文字对话与 Seedance
生成接口的额度/鉴权失败响应；助手按账号池的可用积分选择下一个账号，复用「保存
当前登录态 → 切换并继续 → 重开项目」流程。该功能默认关闭，必须保持本地代理运行；
登录换 Token、积分查询、资源上传等辅助接口不会触发切号。首次启用建议保持 60 秒
冷却时间，用低成本请求验收。

设置页的「模拟自动切换（不切号）」可合成一次 Seedance 额度不足事件，仅检查
候选账号并显示结果，不访问上游、不改变当前登录态，也不会消耗积分。
...
| API 请求日志 | api_*.log | API 网关请求记录（系统弹窗「系统日志」查看） |
...
**A**: 
- 如果账号有 refresh_token（OAuth 登录的账号），点击「刷新」按钮自动续期；JWT 通常约 8 小时属于正常现象，长期凭据是 refresh_token
...
- **保存当前登录态**：登录豆包后点击右上角「保存当前登录态」，工具自动探测当前账号 user_id 并预填（读取豆包 `User Data\Local State` 中各 Profile 的 saman 账号块，取最近活跃者——客户端切换登录后即时更新；识别失败时兜底最近保存记录；仍失败可手动输入），可选设置别名；保存时先关闭豆包 → 白名单快照 → 重启。
...
- **对话数据独立备份/恢复**：账号行「备份对话数据」图标（备份箱图标）把本机的 IndexedDB / DoubaoStorage 客户端状态独立备份到 `data/doubao_chats/<uid>/`（与快照解耦、覆盖式更新，自动先关闭豆包）；「恢复对话数据」图标把备份回写本机豆包（同样自动先关豆包），适合重装系统 / 重装豆包 / 清理数据目录后快速还原。已有备份的账号在「快照」列显示「对话已备份」徽标（悬停看备份时间与文件数）。**说明：对话正文保存在豆包云端、跟账号走**——本地备份的是客户端状态，恢复后打开豆包会自动从云端同步完整对话记录。
- **对话记录导出**：账号行「导出对话」图标（下载图标）通过豆包官方接口拉取最近会话与逐会话消息，生成 `doubao_chats_<uid>_<时间>.md`（可读版）与 `.json`（结构化存档）到 `data/exports/`。需要账号已录入完整会话凭证（sessionid / sid_guard / **ttwid**——开启代理后豆包流量经过代理即自动写入全部三项，ttwid 也可在编辑弹框手动粘贴）；会话较多时导出需要一些时间（逐会话翻页拉取）。导出文件仅存本地，注意妥善保管。
...
- **续期巡检（高级）**：账号编辑弹框可查看/修改 sessionid / sid_guard / ttwid（已存凭证自动回填；清空 sessionid 与 sid_guard 后保存即删除，ttwid 仅在非空时更新；凭证等同密码，仅存本地），有凭证的账号可运行「探活巡检」：携带凭证访问保活端点（默认 `https://www.doubao.com/info/v2/`，可配置），自动回写续期后的新值、标记已过期账号。若所有账号都未录入凭证，巡检会自动改走保活方式（启动豆包完成会话刷新），不会空转。**凭证自动获取**：代理开启且豆包流量经过代理时，代理会自动从 Cookie 提取 sessionid / sid_guard / ttwid 并自动回写到当前登录账号（账号管理页每 20 秒检查一次，有变化才写入）；编辑弹框也保留「从代理抓包自动填充」按钮手动触发。豆包桌面客户端 cookie 为客户端级加密，无法直接提取，代理抓包是唯一推荐方式。注意：客户端重新登录会使已存 sessionid 轮换失效（对话导出等功能会报错），重新开启代理访问豆包即可自动更新。
