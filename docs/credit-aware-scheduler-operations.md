...
视频路由的安全门与 Core quota reservation 是同一条边界：默认 `paused` 时，直接视频和
`seedance` Chat 两个入口都必须在预占前拒绝；一次性诊断只允许绑定已有普通 Key 的内部
标识与精确请求哈希，并在首次成功匹配后自动失效。此开关不是公网部署的长期绕过方式。

上游只返回“已接受”或完成状态并不足以结算。路由必须取得任务级、可重复核验的 `credits`
回执，校验 task ref、单位、精度、非负值和预占上限后才提交实际额度；否则保持 hold，标记
`reconcile_required`，由 15 秒对账器继续观察。下载视频内容本身不触发结算，重复轮询和进程
重启不得新增账本记录。

本阶段的生产准备仅包括本地 fake bridge 验证、健康检查和公网模板审阅。所有 target、日志、
fixture 与验收报告写入 `D:\gpt`；未完成一次受控真实诊断并审阅脱敏回执前，公网视频保持暂停，
不得以余额变化、token、时长、金额或固定估算替代实际积分单位，也不得启动固定点迁移。
