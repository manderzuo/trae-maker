...
**目标：** 在现有 Core v9 jobs/leases 基础上增加可证明的持久领取、用户公平性、心跳恢复和
排队取消边界，不猜测真实上游接口。
...
- [ ] 为 jobs 增加稳定队列序号/领取状态所需的最小 schema migration（不复制 prompt/body）。
- [ ] 先写两用户 round-robin、同 job 单领取、资源类型隔离和重复提交测试。
- [ ] 实现 `enqueue/claim` owner/状态/lease 校验，领取失败不得产生部分写入。

### Task 2：心跳、恢复、排队取消与显式对账边界

- [ ] 先写 heartbeat 超时、重启 recovery、queued cancel、accepted cancel 和 unknown hold 测试。
- [ ] 复用现有 lease recovery/settlement，确保 job/attempt/request/reservation 终态一致。
- [ ] 仅显式终态/受限证据允许 reconcile；重复 reconcile/release 幂等。

### Task 3：Tauri/worker 接入（Mock only）

- [ ] 增加最小 worker helper 和管理员安全投影，生产 adapter 缺失仍返回 501。
